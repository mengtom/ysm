(function (global) {
    ...
    layList.tableList = function (odj, url, data, limit, size,boxids,is_tables) {
     ...
     //初始化 layui table
    layList.tableInfo = function (odj, url, data, page,limit, size,boxids,is_tables,call_back) {
        var page = page || false ,limit = limit || 0, size = size || 'sm', $data = [], that = this,boxids=boxids || this.boxids,call_back=call_back || false;
        switch (typeof data) {
            case 'object':
                $data = data;
                break;
