<?php /*a:6:{s:58:"F:\WWW\yeshai\app\platform_us\view\crm\crm_order\index.php";i:1606271509;s:52:"F:\WWW\yeshai\app\platform_us\view\public\header.php";i:1605782084;s:61:"F:\WWW\yeshai\app\platform_us\view\public\header_navigate.php";i:1606215127;s:52:"F:\WWW\yeshai\app\platform_us\view\public\footer.php";i:1592563283;s:58:"F:\WWW\yeshai\app\platform_us\view\public\inner_footer.php";i:1595304974;s:56:"F:\WWW\yeshai\app\platform_us\view\public\layui_page.php";i:1595310927;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<!-- <script>
 $eb = parent._mpApi;
	...
	<div class="h-100 w-100 order-order-p-en" id="app">
		<!--头部-->
		<!--头部-->
		...
		<div class="w-1200 mt-80 mb-10 tip  text-666 size-14">
			<a class="text-57">Home /</a> <span class="text-c01f5e">Order Management </span>
		</div>

		<div style="height: 34px;" class="w-1200">
			<div class="w-100 h-100 d-flex align-items-center justify-content-start tab relative">
				<div class="h-100 w-115px text-center ellipsis-1 cursor tab-item relative" :class="{'tab-item-active':where.status===item.value}" @click="getitems(item.value,1)" type="button" v-for="item in orderStatus">{{item.name}}<span v-if="item.count!=undefined" >({{item.count}})</span></div>
			</div>
		...
		<div class="w-1200 relative" style="border: 1px solid #d9d9d9;z-index: 1">
					...
					<form class="w-100 h-100 layui-form  d-flex align-items-start justify-content-between" action="" method="get">
						...
						<div class="h-100 pl-20 d-flex align-items-center justify-content-start flex-wrap mt-20 mb-20">
							...
							<div class="h-100 w-300px mr-30 mb-10">
								...
								<input class="ivu-input w-150px" placeholder="Please enter" type="text" v-model="where.ordersn">
							</div>

							<div class="h-100 w-300px mr-30 mb-10">
								<div class="w-100">User information</div>
								<input class="ivu-input w-150px" placeholder="Please enter" type="text" v-model="where.nickname">
							</div>

							<div class="h-100 w-300px mr-30 mb-10">
								<div class="w-100">Organization/Doctor information</div>
								<input class="ivu-input w-150px" placeholder="Please enter" type="text" v-model="where.institu">
							</div>
							<div class="h-100 w-300px mr-30 mb-10">
								<div class="w-100">Order time</div>
								<input class="ivu-input w-150px" type="text" placeholder="Star → End" style="width: 240px;" id="test13" v-model="where.time">
							</div>
						</div>
			...
			<div class="w-100 label-list bg-fff">
				<div class="w-100 list-title d-flex align-items-center justify-content-between mb-20 pt-20">
					<span class="size-16  ml-20"><span class="fw">All orders</span> </span>
				</div>
						...
						<div v-for="i in items" class="w-100 d-flex align-items-center justify-content-start flex-column t-tabl-top">
							...
							<div class="w-100 d-flex align-items-center justify-content-start t-tabl-top-centen">
								...
								<div class="h-100 d-flex align-items-center justify-content-center flex-column w-95px bl">
									...
									<span class="w-100 text-center size-12 text-57" style="line-height: 1;">Freight ${{i.pay_postage}}元</span>
								</div>
								...
								<div class="h-100 d-flex align-items-center justify-content-center w-90px flex-column bl">
									...
									<span class="w-100 text-center text-27"><a :href=`<?php echo Url('order_details'); ?>?id=${i.id}` class="text-c01f5e">Details</a></span>
								</div>
								<div class="h-100 d-flex align-items-center justify-content-center flex-column w-90px bl pl-10 pr-10">
								<!-- 	<button v-if="i._status == 2 && i.shipping_type==1" type="button"  class="ivu-btn ivu-btn-primary ivu-btn-small mt-8 shipments" style="width: 72px;" @click="shipments(i.id)">发货</button> -->
									...
									<span>Customs declaration success</span>
								</div>
							</div>
							<div class="w-100" style="display:none">
								<div class="" style="width: 1128px;margin: auto">
									<div class="w-100 text-57 mt-8"><span>Types of microtabs：{{i.micro_cate_count}}</span><span class="ml-20">Total number of microtabs：{{i.micro_num}}</span></div>
		...
		<div style="display: none" class="modal-root label-modal order-modal">
			<div class="ant-modal-wrap"></div>
			<div class="ant-modal fixed" style="height: 422px;width: 510px;">
				<div class="w-100 top d-flex align-items-center justify-content-start size-16  relative mb-20">
				...
				<form class="layui-form" action="">
					<div class="items  d-flex align-items-center justify-content-start flex-column w-100">
						<div class="item w-100 d-flex align-items-start justify-content-start">
							<div class="fw" style="margin-left: 40px;">Refund method：</div>
						</div>

						<div class="item mt-20 w-100">
							<div class="d-flex align-items-start flex-column justify-content-start" style="margin-left: 40px;min-height: 32px;">
								<div class="w-100">
									<input class="h-100" type="radio" value="1" checked name="demo1" title="Direct refund">
								</div>
								<div class="size-12 text-center remark text-57 w-auto">
						...
						<div class="item w-100">
							<div class="d-flex align-items-start justify-content-start flex-column" style="margin-left: 40px;min-height: 32px;">
								<div class="w-100">
...
<script type="text/javascript">
	...
	var app   = new Vue({
		...
		data   : {
			...
			orderStatus: [
                ...
                {name: 'Completed', value: 3,count:orderCount.jy},
	...
	// 	var url="<?php echo Url('update_delivery'); ?>"+'?id='+e;
