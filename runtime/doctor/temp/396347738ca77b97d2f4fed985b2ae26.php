<?php /*a:4:{s:58:"F:\WWW\yeshai\app\doctor\view\ts\ts\microchip_selected.php";i:1603791340;s:47:"F:\WWW\yeshai\app\doctor\view\public\header.php";i:1602238877;s:47:"F:\WWW\yeshai\app\doctor\view\public\footer.php";i:1603783219;s:53:"F:\WWW\yeshai\app\doctor\view\public\inner_footer.php";i:1603783219;}*/ ?>
<!DOCTYPE html>
<html lang="zh_CN">
<head>
    <meta charset="UTF-8">
										...
										<div style="width: 210px;" class="ml-20 mt-10  d-flex align-items-center justify-content-start">
											...
											<select v-model="where.cate1">
												...
												<?php if(is_array($cate2) || $cate2 instanceof \think\Collection || $cate2 instanceof \think\Paginator): $i = 0; $__LIST__ = $cate2;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($i % 2 );++$i;?>
													<option :value="<?php echo htmlentities($vo2['id']); ?>"><?php echo htmlentities($vo2['title']); ?></option>
											...
											<div class="w-150px">
												<select v-model="where.cate2">
													...
													<?php if(is_array($cate1) || $cate1 instanceof \think\Collection || $cate1 instanceof \think\Paginator): $i = 0; $__LIST__ = $cate1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($i % 2 );++$i;?>
														<option :value="<?php echo htmlentities($vo1['id']); ?>"><?php echo htmlentities($vo1['title']); ?></option>
													<?php endforeach; endif; else: echo "" ;endif; ?>
												</select>
											</div>
										</div>
											...
											<div class="w-150px">
												<select v-model="where.cate3">
													...
													<?php if(is_array($cate3) || $cate3 instanceof \think\Collection || $cate3 instanceof \think\Paginator): $i = 0; $__LIST__ = $cate3;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo3): $mod = ($i % 2 );++$i;?>
														<option :value="<?php echo htmlentities($vo3['id']); ?>"><?php echo htmlentities($vo3['title']); ?></option>
													<?php endforeach; endif; else: echo "" ;endif; ?>
												</select>
											</div>
										</div>
						...
						<div class="mt-10 items1" style="width: 754px;margin: auto">
							...
							<div class="">
								...
								<div v-for="(i,k) in items1" class="w-100  d-flex align-items-center justify-content-start" style="height: 38px;border-bottom: 1px solid #f9f9f9;">
									...
									<div class="w-100px ellipsis-1" :title="i.price">{{i.sell_price}}</div>
									<div class="w-100px ellipsis-1" :title="i.status">{{i.status === 1 ? "可用":"不可用"}}</div>
									<div class="w-50px text-center">
									<i :data-name="i.zn_name" :data-pid="i.id" :data-id="i.micro_id" :data-kind="i.id" :data-scope="i.dose" :data-maxnum="i.dose_max" :data-unit="i.unit" data-val="1" :data-dosage="i.dose" :data-dosage1="i.dose" :data-rmbprice="i.sell_price >0 ? i.sell_price :i.platform_price" :data-type="1" :data-status="
									i.status" class="icon iconfont iconicon-test text-0081a7 cursor items-add"
									></i>
									</div>
								</div>
							</div>
								...
								<div class="h-100">
									<div class="d-flex align-items-center justify-content-start flex-wrap pt-10">
										...
										<div style="width: 220px;" class="ml-20 mt-10  d-flex align-items-center justify-content-start">
											...
											<div class="w-150px">
												<i-select size="small" v-model="where1.cate_id">
							...
							<div class="">
								<div v-for="(i,k) in items2" class="w-100  d-flex align-items-center justify-content-start flex-column" style="min-height: 38px;border-bottom: 1px solid #f9f9f9;">
									<div class="w-100 d-flex align-items-center justify-content-start" style="height: 38px;">
										...
										<div class="w-100px ellipsis-1" :title="i.is_status">{{i.is_status === 1 ? "可用":"不可用"}}</div>
										<div class="w-50px text-center">
											<i :data-name="i.name_zn" :data-pid="i.id"  class="icon iconfont iconicon-test text-0081a7 cursor pf-items-add"></i>
										</div>
									</div>
									...
									<div class="w-100 bg-f6 pl-30" style="min-height: 30px;padding: 4px;display:none;">
										<div class="w-100 h-auto d-flex align-items-center justify-content-start flex-wrap pl-30 pr-30">
											<span class="mr-8 wplistspan" v-for="m in i.microchip"
													...
													:data-val="m.num" :data-status="m.status" :data-dosage="m.dose" :data-dosage1="m.dose" :data-rmbprice="m.price" :data-type="2"
											>
												<span>{{m.name_zn}}</span>
	...
	var app = new Vue({
		...
		methods: {
			...
			search() {
				var data = this.where;
				var that = this;
				axios.post("<?php echo Url('microchip.microchipProduct/product_list'); ?>", data).then(function (e) {
			...
			search1() {
				...
				var that = this;
				axios.post("<?php echo Url('ts_selected'); ?>", data).then(function (e) {
