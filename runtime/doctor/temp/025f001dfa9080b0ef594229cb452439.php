<?php /*a:4:{s:45:"F:\WWW\yeshai\app\doctor\view\ts\ts\index.php";i:1603784444;s:47:"F:\WWW\yeshai\app\doctor\view\public\header.php";i:1602238877;s:47:"F:\WWW\yeshai\app\doctor\view\public\footer.php";i:1603783219;s:53:"F:\WWW\yeshai\app\doctor\view\public\inner_footer.php";i:1603783219;}*/ ?>
<!DOCTYPE html>
<html lang="zh_CN">
<head>
    <meta charset="UTF-8">
								...
								<div class="h-100 d-flex align-items-center justify-content-start mr-20 mt-20">
									...
									<div class="w-150px ml-8">
										...
										<i-select v-model="cate2">
											...
											<i-option value="<?php echo htmlentities($c2['id']); ?>"><?php echo htmlentities($c2['title']); ?></i-option>
								...
								<div class="h-100 d-flex align-items-center justify-content-start mr-20 mt-20">
									...
									<div class="w-150px ml-8">
										...
										<i-select v-model="cate3">
											...
											<i-option value="<?php echo htmlentities($c3['id']); ?>"><?php echo htmlentities($c3['title']); ?></i-option>
											<?php endforeach; endif; else: echo "" ;endif; ?>
										</i-select>
									</div>
								</div>
					...
					<table class="ivu-table  ivu-table-default" style="overflow: initial">
						...
						<tr class="cursor text-57" v-for="i in data">
							...
							<td class="ckitem" v-if="i.type == 1">
								<a class="text-0081a7" :href=`<?php echo url('details'); ?>?id=${i.id}`>
									<i class="icon iconfont iconbian-ji"></i>
									详情
								</a>
								<a class="text-27" :href=`<?php echo url('edit'); ?>?id=${i.id}`>
									<i class="icon iconfont iconbian-ji"></i>
									编辑
								</a>
								<a class="text-0081a7" :href=`<?php echo url('delete'); ?>?id=${i.id}`>
		...
		<div style="display: none;" class="modal-root ts-edit-modal">
			...
			<div class="ant-modal fixed" style=" width: 425px;height: 193px;">
				...
				<div class="w-100 d-flex align-items-center justify-content-end foot">
					<a href="<?php echo Url('create'); ?>"  class="ivu-btn-primary ivu-btn mr-10">新建配方</a>
			...
			<div class="ant-modal fixed" style=" width: 425px;height: 243px;">
				...
				<div class="w-100 d-flex align-items-center justify-content-center flex-column">
					...
					<div class="w-100 mt-20 mb-30 text-center">
						<a data-href="<?php echo Url('prescription'); ?>" class="ivu-btn-primary ivu-btn mr-30 route">确定，生成患者处方</a>
						<a data-href="<?php echo Url('purchase'); ?>" class="ivu-btn-primary ivu-btn route">确定，我要自己采购</a>
					</div>
				</div>
				<div class="w-100 d-flex align-items-center justify-content-end foot">
					<button type="button" class="ivu-btn-default ivu-btn closemode mr-20">取消</button>
	...
	var app = new Vue({
		...
		methods: {
			...
			getitems() {
				...
				axios.post("<?php echo Url('ts_list',['type'=>$type]); ?>", data).then((e) => {
					e=e.data;
					this.data = e.data;
					this.total= e.count;
				});
