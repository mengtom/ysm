...
if(!function_exists('array_reset_struct')){
    /**
     * 获取数组某个字段的值或者叠加的值
     * @return [type] [description]
     */
    function array_reset_struct($data,$field=''){
        static $info=[];
...
if(!function_exists('array_sort')){
     ...
     * @return array
     */
    function array_sort($arr, $keys, $type = 'desc',$re=false)
    {
        ...
        foreach ($key_value as $k => $v) {
            $re ? $new_array[]=$arr[$k] : $new_array[$k] = $arr[$k];
        }
        return $new_array;
    }
}