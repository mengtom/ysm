    ...
    public static function getModelObject($where=[]){
        ...
        if(!empty($where)){
            ...
            if(isset($where['keyword']) && $where['keyword']!=''){
                $model = $model->where('name_zn|code|name_en','LIKE',"%{$where['keyword']}%");
            }
            if(isset($where['cate2']) && trim($where['cate2'])!=''){
                $catid1 = $where['cate2'].',';//匹配最前面的cateid
                $catid2 = ','.$where['cate2'].',';//匹配中间的cateid
                $catid3 = ','.$where['cate2'];//匹配后面的cateid
                $catid4 = $where['cate2'];//匹配全等的cateid
//                $model = $model->whereOr('p.cate2','LIKE',["%$catid%",$catidab]);
                $sql = " LIKE '$catid1%' OR `cate_id` LIKE '%$catid2%' OR `cate_id` LIKE '%$catid3' OR `cate_id`=$catid4";
                $model->where(self::getPidSql($where['cate2']));
            }
    ...
    public static function TsMicorchipList($where){
        $model=self::getModelObject($where)->field('*,FROM_UNIXTIME(add_time,"%Y-%c-%d %h:%i") as add_time');
        if($where['limit'] > 0 && $where['page'] > 0) $model=$model->page((int)$where['page'],(int)$where['limit']);
        $data=($data=$model->select()) && count($data) ? $data->toArray():[];
        foreach ($data as &$item){
