...
use think\facade\Db;
// use app\admin\model\microchip\MicrochipProduct as MicrochipModel;
class TsMicrochip extends BaseModel
{
    ...
    public static function addMicrochip($ts_id,$data=array(),$num=array()){

    }
    public static function getAll($ts_id){
        $model=new self;
        $model=$model->alias('tm')->join('MicrochipProduct m','m.id = tm.micro_id','right')->where(['tm.ts_id'=>$ts_id])->field(['tm.*,m.zn_name,m.id,m.dose_min,m.dose_max,m.unit,m.price,m.usd,m.is_show as is_status'])->select();
        return $model->toArray();
    }
    // public static function getingredient($id){
    //     $ingre1=$ingre2=0;
    //     $ingre1+=Db::name('microchip_product_ingredient')->where(['microchip_id'=>$id,'type'=>1])->count();
    //     $ingre2+=Db::name('microchip_product_ingredient')->where(['microchip_id'=>$id,'type'=>2])->count();
    ...
    //     }unset($vv);
    //     sort($ingre);
    //     return compact($ingre1,$ingre2,$ingredient);
    // }
}