...
class SystemPlatformConfig extends BaseModel
{
     ...
     * @var string
     */
    protected $name = 'platform_system_config';
     ...
     * @return array
     */
    public static function getMore($menus,$p_id=0)
    {
        $model = new self;
        $menus = is_array($menus) ? implode(',',$menus) : $menus;
        $model = $p_id ? $model->where('platform_id',$p_id): $model;
        $list = $model->where('menu_name','IN',$menus)->column('value','menu_name')?:[];
     ...
     * @throws \think\exception\DbException
     */
    public static function getAll($id,int $platform_id,int $status = 1){
