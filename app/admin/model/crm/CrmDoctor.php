...
class CrmDoctor extends BaseModel
{
    ...
    public static function getModelObject($where=[]){
        ...
        if(!empty($where)){
            ...
            if(isset($where['type']) && (int)$where['type']){
                $model = $model->where('p.category',$where['type']);
            }
    ...
    public static function DoctorList($where,$fieldout=[]){
        ...
        foreach ($data as &$item){
            ...
            if($item['category'] == 2){
                $item['total_ts']=Db::name('crm_order')->where(['order_type'=>1,'institu_leader'=>$item['id'],'platform_leader'=>$item['platform_leader']])->count();
                $item['total_order']=Db::name('crm_order')->where(['institu_leader'=>$item['id'],'platform_leader'=>$item['platform_leader']])->where('status','>=','1')->where('order_type','IN',[0,2])->count();
                $item['total_price']=Db::name('crm_order')->where(['institu_leader'=>$item['id'],'platform_leader'=>$item['platform_leader']])->where('order_type','IN',[0,2])->where('status','>=','1')->sum('pay_price');
            }else{
                $item['total_ts']=Db::name('crm_order')->where(['order_type'=>1,'user_id'=>$item['uid']])->count();
                $item['total_order']=Db::name('crm_order')->where(['user_id'=>$item['uid']])->where('status','>=','1')->where('order_type','IN',[0,2])->count();
                $item['total_price']=Db::name('crm_order')->where(['user_id'=>$item['uid']])->where('order_type','IN',[0,2])->where('status','>=','1')->sum('pay_price');
            }
     ...
     */
    public static function addDoctortion($data){
        if(count($data['ca']) > 0 && isset($data['ca'])) $data['ca']=serialize($data['ca']);
        $data['createtime'] = time();
        $data['add_ip']=app('request')->ip();
        $data['plat_type'] = 'platform';
        $data['category']= 3;
        $res = self::field('name,user_id,referrer,refer_mobile,refer_email,ecp,ecm,ece,now_money,currency,createtime,add_ip,plat_type,ca,business_license,place,group,platform_leader,category')->insert($data);
        return $res;
    }
    /**
     * 获取单个平台信息
