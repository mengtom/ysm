...
class CrmPlatformTs extends BaseModel
{
    ...
    public static function getModelObject($where=[]){
        $model=new TsModel;
        $model=$model->alias('pi')->join("CrmPlatformTs i",'i.ts_id=pi.id and i.platform_id= '.$where['id'],'left')->field('pi.*,i.platform_id as pid,i.status');
        if(!empty($where)){
            $model=$model->group('pi.id')->where(['pi.is_del'=>0,'pi.type'=>0]);
