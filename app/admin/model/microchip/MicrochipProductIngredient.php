...
class MicrochipProductIngredient extends BaseModel
{
    ...
    public static function createIngredient($result,$microchip_id,$type=0)
    {
        ...
        foreach ($result as $index=>$value){
            ...
            $value['type']=$type;
            $value['price_rmb']=$product['price'] ?sprintf("%.3f",$product['price'] /1000):0;
            $value['price_usd']=$product['price'] && $product['scale'] > 0? sprintf("%.3f",$product['price'] / $product['scale'] / 1000) :0;
            $result[$index] = $value;
        }unset($value);
