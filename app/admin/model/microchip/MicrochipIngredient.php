...
// use app\admin\model\microchip\StoreCategory as CategoryModel;
...
//        return $res;
     ...
     * @return [type]         [description]
     */
    public static function getTotalCost($result=[],$type=false){
        $cost_price=0;
        foreach($result as $k=> $item){
            ...
            if($info['scale'] > 0 && $type){
                $price=sprintf('%.5f',$price/$info['scale']);
            }
            if($item['dose'] > 0){
                $price=$price*$item['dose'];
            }
            $cost_price+=$price;
        }unset($item);
        return sprintf("%.4f",$cost_price);
    }
}