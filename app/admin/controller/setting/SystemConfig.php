...
class SystemConfig extends AuthController
{
   ...
   public function index(){
       ...
       $formbuider = [];
       $list->toArray();
       $logisgics=\app\admin\model\system\SystemConfigLogistics::getAll($tab_id);
       if($logisgics)
          $logisgics->toArray();
       ...
       $this->assign(compact('form','logisgics'));
   ...
   public function save(){
    ...
    public function save_basics(){
        ...
        if($request->isPost()){
            $post = $request->post();
            if(!$post['microchip_processing_rmb'])
              return $this->failed('微片加工费不能为空');
            if(!$post['packing_labor_rmb'])
              return $this->failed('包装人工费不能为空');
            // foreach ($post as $k=>$v){
            //     if(is_array($v)){
            ...
            //     }
            // }
            $config_tab_id=$post['config_tab_id'];
            unset($post['config_tab_id']);
            foreach ($post as $k=>$v){
                if(!is_numeric($k)){
                  ConfigModel::edit(['value' => json_encode($v)],$k,'menu_name');
                }else{
                  $logisgics=new \app\admin\model\system\SystemConfigLogistics;
                  $logisgics::where(['weight'=>$k,'config_tab_id'=>$config_tab_id])->data($v)->update();
                }
            }
            CacheService::delete(SystemConfigService::CACHE_SYSTEM);
