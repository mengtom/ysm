...
class Index extends AuthController
{
    public function index()
    {
        ...
        //本月订单总金额
        $topData['now_month_order_p'] = CrmOrderModel::where('paid', 1)->where('order_type','IN',[0,2])->whereTime('pay_time', 'month')->sum('pay_price');
        //本月订单新增
        $topData['now_order_info_c'] = CrmOrderModel::where('order_type','IN',[0,2])->where('add_time', '>', $now_month)->count();
        //医生总数量
        $doctor_c = CrmUserModel::where('type',1)->count();
        $now_month_doctor_c=CrmUserModel::where('type',1)->where('createtime', '>', $now_month)->count();
        $topData['doctor']=['doctor_c'=>$doctor_c,'month_doctor_c'=>$now_month_doctor_c];
        //患者总数
        $patient_c = CrmUserModel::where('type',0)->count();
        $now_month_patient_c=CrmUserModel::where('type',0)->where('createtime', '>', $now_month)->count();
        $topData['patient']=['patient_c'=>$patient_c,'month_patient_c'=>$now_month_patient_c];
        //医疗机构数量
        $institu_c = CrmUserModel::where('type','IN',[2,3])->count();
        $now_month_institu_c=CrmUserModel::where('type','IN',[2,3])->where('createtime', '>', $now_month)->count();
        $topData['institu']=['institu_c'=>$institu_c,'month_institu_c'=>$now_month_institu_c];
        $this->assign([
