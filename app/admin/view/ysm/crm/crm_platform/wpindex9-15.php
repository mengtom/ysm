			...
			<div class="w-100 table ">
				<form action="" class="">
					...
					<script type="text/html" id="status" lay-filter="status">
						<input type='checkbox' lay-skin='primary' lay-filter='checkboxIsSelected' {{ d.status == 1 ? 'checked' : '' }} class='checkboxIsSelected' value=' {{ d.status == 1 ? d.id.1 : d.id.0 }}'>
					</script>
				</form>
			</div>
			<!--page 分页-->
...
<script type="text/javascript">
    ...
    layList.tableList('list',"{:Url('platform_microList',array('id'=>$platform.id))}",function (){
        ...
        join=[
            ...
            {field: 'cost_rmb', title: '成本价',templet:'#price',align:'center', event:'price',width:'8%'},
            {field: 'price', title: '建议售价',templet:'#sell_price',align:'center', event:'sell_price',width:'8%'},
            {field: 'platform_price', title: '平台结算价',templet:'#platform_price',edit:'platform_price',align:'center',width:'9%'},
            {type:'checkbox',filed:'select'},
        ];
        return join;
    })
