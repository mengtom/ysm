{include file="public/header"}
	<title>CMR-平台管理-查看平台内患者</title>

	<div class="h-100 w-100 cmr-patient" id="app">
		<!--头部-->
		{include file="public/header_navigate"}
		<!--头部-->
		<div class="w-1200 mt-80 mb-10 tip  text-666 size-14">
			<a class="text-57">首页 /</a> <a href="index.html" class="text-57">平台管理 </a> <span class="text-c01f5e">/ 查看平台内患者 </span>
		</div>
		<!--搜索区-->
					...
					<div class="h-100 pl-20 d-flex align-items-center justify-content-start flex-column mt-20 mb-20">
						...
						<div class="w-100 d-flex align-items-center justify-content-start">
							...
							<div class="h-100  d-flex align-items-center justify-content-start ml-20">
								<span class="w-100px">所属机构/医生:</span>
								<input class="ivu-input" style="width: 292px;" placeholder="请输入" type="text" v-model="institu">
							</div>
						</div>
		...
		<div class="w-1200 label-list mt-20 mb-30 bg-fff">
			<div class="w-100 list-title d-flex align-items-center justify-content-between mb-20 pt-20">
				<span class="size-16  ml-20"><span class="fw">平台内患者</span> <span class="size-14">（{$platform.p_name}）</span> </span>
				<div>
			...
			<div class="w-100 table ">
				...
				<table class="ivu-table  ivu-table-default" style="overflow: initial">
					...
					<tr class="ivu-table-header cursor">
						...
						<th class="bg-f6 w-300px ">所属平台</th>
					...
					<tr v-for="i in data" class="cursor text-57">
						<td class="pl-30 ckitem">{{i.user_id}}</td>
						<td class="ckitem">{{i.nickanme}}</i></td>
						<td class="ckitem">{{i.account}}</td>
						<td class="ckitem">{{i.createtime}}</td>
						<td class="ckitem">{{i.institu_name}}</td>
						<td class="ckitem ">{{i.plat_name}}</td>
	...
	var app   = new Vue({
		...
		methods: {
			...
			getitems() {
				...
				axios.post('{:Url("patient_list")}', data).then((e) => {
