{include file="public/header"}
	...
	<div id="app" class="ts-edit">
		...
		<div class="w-1200 tip mt-80  text-666 size-14">
			<span>首页 /</span> <a class="text-57" href="index.html"> TS配方 / </a> <span class="text-c01f5e"> 编辑配方 </span>
		</div>
		<!--列表-->
		<div class="w-1200  wp-edit mt-10 pb-10 mb-30" style="border-radius: 1px;">
			<form class="d-flex layui-form  align-items-start justify-content-between" action="{:Url('save',array('id'=>$info['id']))}" method="post">
					...
					<div class="items pl-30 w-100">
						...
						<div class="item  w-100 d-flex align-items-center justify-content-start  ">
							<div>
								...
								<div class="w-100">
									<input type="text" placeholder="由中文/英文/数字/符号组成" class="ivu-input w-400px ml0" name="name_zn"  value="{$info.name_zn}">
								</div>
							</div>
							<div style="margin-left: 40px;">
								<div class=" size-14 item-text">Name</div>
								<div class="w-100">
									<input type="text" placeholder="由英文/数字/符号组成" class="ivu-input w-400px ml0" name="name_en" value="{$info.name_en}">
								</div>
							</div>
						</div>
						...
						<div class="w-100 d-flex align-items-center justify-content-start flex-column  hxcflist">
							...
							<div v-for="(i,k) in hxcflistdata" class="w-100 d-flex align-items-center justify-content-around hxcfitem" style="height: 40px;">
								...
								<div class="d-flex align-items-center justify-content-start w-150px ">
									<input class="ivu-input ivu-input-small" :value="i.val" :data-num='i.val'  :name="`mid[${i.id}]`" min="1" type="text" min="1" style="width: 70px">
								...
								<div class="d-flex align-items-center justify-content-center w-50px">
									<i class="icon iconfont iconcha text-c01f5e cursor rmhxcfitem" :data-key="k"></i>
								</div>
							</div>
						</div>
				...
				<div class="ts_right d-flex ml-20 h-auto d-flex align-items-start justify-content-start flex-column">
					<div class="top w-100 bg-fff" style="min-height:413px;height:auto;">
        ...
        <div class="ant-modal fixed" style="width : 852px ;height: 520px ;">
            <div class="w-100 top d-flex align-items-center justify-content-start size-16  relative">
                ...
                <span style="right: 20px;" class="absolute cursor 	closemode-facts">
					<i class="icon iconfont iconguanbi text-bf"></i>
				</span>
            </div>
            ...
            <div class="items w-100">
                <div class="w-100 h-100">
                    <div class="w-100 pt-10 pb-10 bg-f4 pl-20">
                        <div class="w-100 size-20 fw">Supplement Facts</div>
                        <div class="w-100">Serving Size 1 Package</div>
                    </div>
                    <div class="h-32px w-100 d-flex b-border pl-20">
                    ...
                    <div v-for="i in factsdata" class="h-32px w-100 d-flex b-border item pl-20">
                        <div class="w-60 h-100 d-flex align-items-center justify-content-start">Vitamin D</div>
                        <div class="w-20 h-100 d-flex align-items-center justify-content-end">220 iu</div>
                        <div class="w-20 h-100 d-flex align-items-center justify-content-end pr-30">20 %</div>
                    </div>

                    <div class="w-100  pt-20 pb-20 pl-20 pr-20">
                        <span class="fw">Other ingredients:</span>
                        <span style="word-break:break-all">
							VitaminD,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin,Vitamin
						</span>
                    </div>
...
<script type="text/javascript">
	...
	function total() {
		...
		for (var i = 0; i < dest.length; i++) {
			if (parseFloat(dest[i].dosage) >= parseFloat(dest[i].maxnum)) {
				dest[i].val    = Math.ceil(parseFloat(dest[i].maxnum) / parseFloat(dest[i].dosage1));
				dest[i].dosage = parseFloat(dest[i].maxnum);
			}
			wpkind++;
		...
		if(dest) {
			axios.post("{:url('getIngredient')}", {data: dest}).then(e => {
				...
				for (var i = 0; i < arrs.ingre.length; i++) {
						arrs.ingre[i].value=arrs.ingre[i].dose
				}
				...
				$('.hxkind span').html(arrs.ingre1); //活性种类
				$('.flkind span').html(arrs.ingre2);//辅料种类
				app.$data.cflist = dests;//成分列表
			});
    ...
    */
    $('.facts').click(function () {
        ...
        //先清空数据
        app.$data.factsdata = [];
        axios.post('url', {data: arr}).then(function (e) {
            console.log(e)
            if (e) {
                app.$data.factsdata = e;
            }
        })
        console.log(arr)
    });

	$('.closemode-facts').click(function () {
        $('.ts-edit-facts').hide();
	})
