...
use app\institu\model\ump\StoreCouponUser;
use app\institu\model\wechat\WechatUser;
...
class CrmOrder extends BaseModel
{
    ...
    public static function orderCount($order_type = 0,$model = null)
    {
        ...
        $data['dp'] = self::statusByWhere(3, $model)->where(['is_system_del' => 0,'order_type'=>$order_type])->count();
    ...
    public static function OrderList($where)
    {
        ...
        foreach ($data as &$item) {
            ...
            // }
            // $item['_info'] = $_info;
            $micro=Db::name('crm_order_microchip')->where('order_id', $item['id'])->field('micro_code,micro_name,micro_price,num,total_dose,total_price')->select();
            $item['_micro']=$micro;
            $item['micro_num']=count($micro);
            $item['micro_total']=Db::name('crm_order_microchip')->where('order_id', $item['id'])->sum('num');
...
//                    $model = $model->where($aler.'combination_id',">",0)->where($aler.'pink_id',">",0);
        ...
        if (isset($where['keyword']) && $where['keyword'] != '') {
            $model = $model->where($aler . 'order_name', 'LIKE', "%$where[keyword]%");
        }
        ...
        if(isset($where['doctor']) && $where['doctor']){
             $model = $model->where($join . '.doctor_name', $where['doctor']);
        }
        if(isset($where['order_type']) && in_array($where['order_type'],[0,1]))
        {
