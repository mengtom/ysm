...
class CrmPlatform extends BaseModel
{
    ...
    public static function getModelObject($where=[]){
        ...
        if(!empty($where)){
            ...
            if(isset($where['referral_code']) && $where['referral_code'] !=''){
                $model = $model->where('p.referral_code',$where['referral_code']);
            }
    ...
    public static function addPlatform($data){
        if(count($data['ca']) > 0 && isset($data['ca'])) $data['ca']=json_encode($data['ca']);
        $data['createtime'] = time();
        $data['add_ip']=app('request')->ip();
        $data['plat_type'] = 'system';
        $data['category']= 4;
        $res = self::field('p_name,user_id,referrer,refer_mobile,refer_email,ecp,ecm,ece,now_money,currency,createtime,add_ip,plat_type,ca,business_license,place,adminid,default_discount,single_dose,barrel_price,out_pack')->insert($data);
        return $res;
    }
    /**
     * 获取单个平台信息
    ...
    public static function getInstituList($where){
