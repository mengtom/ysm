 ...
 * 菜单  model
 * Class SystemMenus
 * @package app\isntitu\model\system
 */
class SystemMenus extends BaseModel
{
    ...
    {
        $menusList = self::where('is_show','1')->where('module','institu')->where('access','1')->order('sort DESC')->select();
    ...
    {
        static $allAuth = null;
        static $instituAuth = null;
        if($allAuth === null) $allAuth = $platformFilter == true ? SystemRole::getAllAuth() : [];  //所有的菜单
        if($instituAuth === null) $instituAuth = $platformFilter == true ? CrmUsers::activeInstituAuthOrFail() : [];//当前登录用户的菜单
        ...
        foreach ($menusList as $k=>$menu){
            ...
            if($menu['pid'] == $pid){
                ...
                if($pid != 0 && $platformFilter && in_array($authName,$allAuth) && !in_array($authName,$instituAuth)) continue;
