 ...
 * @day: 2020/6/10
 */
namespace app\institu\model\label;
 ...
 * 标签标签model
 * Class LabelCategory
 * @package app\institu\model\wechat
 */
class Label extends BaseModel
{
    ...
    public static function getAll($where = array()){
        ...
        return self::page($model,function($item){
            // if(!$item['mer_id']) $item['institu_name'] = '总后台管理员---》'.SystemAdmin::where('id',$item['institu_id'])->value('real_name');
            // else $item['institu_name'] = Merchant::where('id',$item['mer_id'])->value('mer_name').'---》'.MerchantAdmin::where('id',$item['institu_id'])->value('real_name');
