...
namespace app\institu\model\ts;

// use app\institu\model\ump\StoreBargain;
// use app\institu\model\ump\StoreCombination;
// use app\institu\model\ump\StoreSeckill;
...
class Ts extends BaseModel
{
    ...
    public static function setData($type){
        switch ((int)$type){
            case 1:
                $data = ['is_status'=>1,'is_del'=>0];
                break;
            case 0:
                $data = ['is_status'=>0,'is_del'=>0];
                break;
            // case 3:
            //     $data = ['p.is_del'=>0];
            //     break;
    ...
    public static function getModelObject($where=[]){
        ...
        if(!empty($where)){
            if(isset($where['is_status']) && $where['is_status']!='' && ($data=self::setData($where['is_status']))){
                $model = $model->where($data);
            }
           if(isset($where['type'])){
                switch($where['type']){
                    case 1:
                        $model = $model->where('type','In','0,4');
                    break;
                    case 2:
                        $model = $model->where('type','=',1);
                    break;
                    default:
                        $model = $model->where('type','In','0,3,4');
                    break;
                }
            }
            if(isset($where['keywork']) && $where['keywork']!=''){
                $model = $model->where('name_zn|code|name_en','LIKE',"%$where[keywork]%");
            }
            ...
            if(isset($where['platform_id']) && $where['platform_id'] !=''){
                ...
                if($ts_id){
                     $model = $model->where("id In (".implode(',',$ts_id).") or platform_id = ".$where['platform_id']);
                }else{
            ...
            if(isset($where['order']) && $where['order']!=''){
                ...
                $model = $model->order('add_time desc,id desc');
            }
        }
        return $model;
    }
    ...
    public static function TsMicorchipList($where){
        ...
        $data=($data=$model->select()) && count($data) ? $data->toArray():[];
        foreach ($data as &$item){
            $item['total_price']=$where['currency']? $item['total_usd']:$item['total_price'];
