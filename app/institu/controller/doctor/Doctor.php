<?php
namespace app\institu\controller\doctor;
...
use app\institu\model\crm\CrmUsersGroup as GroupModel;
 ...
 * 医生管理 控制器
 * Class
 * @package app\institu\controller\crm
 */
class Doctor extends AuthController
{
    ...
    {
        $group=GroupModel::groupList(['platform_id'=>$this->instituPId,'type'=>2]);
    ...
    public function save()
    {
        ...
        if($name) return $this->failed('医生已存在');
        $account=CrmUsers::be(['account'=>$data['account']]);
        if($account) return $this->failed('医生已存在');
        $refer=DoctorModel::be(['refer_mobile'=>$data['refer_mobile']]);
        if($refer) return $this->failed('医生已存在');
        ...
        $data['platform_leader'] =$this->instituInfo['platform_leader'];
        $data['institu_leader'] =$this->instituPId;
        $time=time();
        try {
            $default=CrmUsers::addUsers(['account'=>$data['account'],'password'=>$data['password'],'type'=>1,'platform_leader'=>$data['platform_leader'],'platform_name'=>$this->instituInfo['platform_name'],'platform_time'=>$time,'institu_leader'=>$data['institu_leader'],'institu_name'=>$this->instituInfo['i_name'],'institu_time'=>$time,'last_ip'=>app('request')->ip(),'last_login'=>$time]);
        } catch (\Exception $e) {
        ...
        if($res){
            ...
            return $this->successful('添加医生成功');
        }
        BaseModel::rollbackTrans();
        return $this->failed(DoctorModel::getErrorInfo('添加医生失败'));
    }
    ...
    public function update($id)
    {
        ...
        if($data['refer_mobile'] != $doctor['refer_mobile']){
            $refer=CrmUsers::be(['refer_mobile'=>$data['refer_mobile']]);
            if($refer) return $this->failed('医生已存在');
        }
   ...
   /**
    ...
    public function groupList(){
        ...
        $data['platform_id']=$this->instituPId;
    ...
    public function delete(int $id){
        ...
        $doctor = DoctorModel::getDoctorInfo(['id'=>$id]);
        if(!$doctor) return Json::fail('数据不存在!');
        $default=DoctorModel::deleteDoctor($id,$doctor['uid']);
        if($default)
