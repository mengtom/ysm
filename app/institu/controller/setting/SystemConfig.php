<?php
namespace app\institu\controller\setting;
...
use app\institu\model\system\SystemConfig as ConfigModel;
/**
 *  配置列表控制器
 * Class SystemConfig
 * @package app\institu\controller\system
 */
class SystemConfig extends AuthController
{
   ...
   public function index(){
       ...
       $list = ConfigModel::getAll($tab_id,$this->instituPId);
       $platform=$formbuider=[];
       if($type==3){//其它分类
          $config_tab = null;
       }else{
           ...
           foreach ($config_tab as $kk=>$vv){
               $arr = ConfigModel::getAll($vv['value'],$this->instituPId)->toArray();
               // if(empty($arr)){
               //     unset($config_tab[$kk]);
               // }
           }
           ...
           switch ($data['type']){
               ...
               case 'upload'://文件上传
                   switch ($data['upload_type']){
                       case 1:
                           $data['value'] = json_decode($data['value'],true)?:'';
                           $formbuider[] = Form::frameImageOne($data['menu_name'],$data['info'],Url::buildUrl('institu/widget.images/index',array('fodder'=>$data['menu_name'])),$data['value'])->icon('image')->width('70%')->height('500px')->info($data['desc'])->col(13);
                           break;
                       case 2:
                           $data['value'] = json_decode($data['value'],true)?:[];
                           $formbuider[] = Form::frameImages($data['menu_name'],$data['info'],Url::buildUrl('institu/widget.images/index',array('fodder'=>$data['menu_name'])),$data['value'])->maxLength(5)->icon('image')->width('70%')->height('500px')->info($data['desc'])->col(13);
                           break;
                       case 3:
                           $data['value'] = json_decode($data['value'],true);
      ...
      if($type == 0){
          $institu=[
          'name'=>$this->instituInfo['platform_name'],
       ...
       $this->assign(compact('form','platform'));
