...
// use app\institu\model\system\Express;
...
// use app\institu\model\order\StoreOrderStatus;
// use app\institu\model\ump\StorePink;
use app\institu\model\user\User;
// use app\institu\model\user\UserBill;
...
use app\institu\model\crm\CrmOrder as OrderModel;
/**
 * 订单管理控制器 同一个订单表放在一个控制器
 * Class StoreOrder
 * @package app\platform\controller\store
 */
class CrmPrescription extends AuthController
{
    /**
     * @return mixed
     */
    public function index()
    {
        $this->assign([
            ...
            // 'orderCount'=>OrderModel::orderCount(1),
            // 'payTypeCount'=>OrderModel::payTypeCount(),
        ]);
        return $this->fetch();
    }
    /**
    ...
    public function getBadge(){
        ...
        return JsonService::successful(OrderModel::getBadge($where));
    }
    /**
     * 获取订单列表
     * return json
    ...
    public function order_list(){
        ...
        $where['institu_id']=$this->instituPId;
        $where['order_type']=1;
    ...
    public function write_order($verify_code = '',$is_confirm = 0)
    {
        if($this->request->isAjax()){
            ...
            $orderInfo = OrderModel::where('verify_code',$verify_code)->where('paid',1)->where('refund_status',0)->find();
    ...
    public function orderchart(){
        ...
        $orderCount = [
            urlencode('未支付')=>OrderModel::getOrderWhere($where,OrderModel::statusByWhere(0))->count(),
            urlencode('未发货')=>OrderModel::getOrderWhere($where,OrderModel::statusByWhere(1))->count(),
            urlencode('待收货')=>OrderModel::getOrderWhere($where,OrderModel::statusByWhere(2))->count(),
            urlencode('待评价')=>OrderModel::getOrderWhere($where,OrderModel::statusByWhere(3))->count(),
            urlencode('交易完成')=>OrderModel::getOrderWhere($where,OrderModel::statusByWhere(4))->count(),
            urlencode('退款中')=>OrderModel::getOrderWhere($where,OrderModel::statusByWhere(-1))->count(),
            urlencode('已退款')=>OrderModel::getOrderWhere($where,OrderModel::statusByWhere(-2))->count()
        ];
        $model = OrderModel::getOrderWhere($where,new OrderModel())->field('sum(total_num) total_num,count(*) count,sum(total_price) total_price,sum(refund_price) refund_price,from_unixtime(add_time,\'%Y-%m-%d\') add_time')
        ...
        $this->assign(OrderModel::systemPage($where,$this->platformPId));
        $this->assign('price',OrderModel::getOrderPrice($where));
...
//        $f[] = Form::radio('status','状态',$product->getData('status'))->options([['label'=>'开启','value'=>1],['label'=>'关闭','value'=>0]]);
    ...
    public function update($id)
    {
        ...
        $data['order_id'] = OrderModel::changeOrderId($data['order_id']);
        OrderModel::edit($data,$id);
    ...
    public function del_order()
    {
        ...
        if(OrderModel::where('is_del',0)->where('id','in',$ids)->count()) return JsonService::fail('您选择的的订单存在用户未删除的订单，无法删除用户未删除的订单');
        $res=OrderModel::where('id','in',$ids)->update(['is_system_del'=>1]);
        if($res)
    ...
    public function update_delivery($id = 0){
        ...
        //短信发送
        event('ShortMssageSend',[OrderModel::where('id',$id)->value('order_id'),'Deliver']);
        return Json::successful('修改成功!');
    }
    ...
    public function take_delivery($id){
        ...
        try{
            if(!OrderModel::edit($data,$id)) {
                OrderModel::rollbackTrans();
                return Json::fail(OrderModel::getErrorInfo('收货失败,请稍候再试!'));
            }else{
    ...
    public function updateRefundY($id){
        ...
        $resEdit = OrderModel::edit($data,$id);
        $res = true;
        if($resEdit){
            $data['type'] = $type;
    ...
    {
        if(!$oid || !($order = OrderModel::get($oid)))
            return $this->failed('订单不存在!');
    ...
    public function offline($id){
        $res = OrderModel::updateOffline($id);
        if($res){
            event('StoreProductOrderOffline',[$id]);
            StoreOrderStatus::setStatus($id,'offline','线下付款');
            return Json::successful('修改成功!');
        }else{
            return Json::fail(OrderModel::getErrorInfo('修改失败!'));
        }
    }
    /**
     * 修改积分和金额
