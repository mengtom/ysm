...
use app\institu\model\store\StoreProduct;
use app\institu\model\crm\SystemConfig;
use app\institu\model\crm\SystemMenus;
use app\institu\model\crm\SystemRole;
use app\institu\model\crm\CrmOrder as CrmOrderModel;//订单
use app\institu\model\user\User;
use app\institu\model\user\UserExtract as UserExtractModel;//分销
use app\institu\model\crm\CrmUsers as CrmUserModel;//用户
// use app\institu\model\store\StoreProductReply as StoreProductReplyModel;//评论
// use app\institu\model\store\StoreProduct as ProductModel;//产品
...
class Index extends AuthController
{
    public function index()
    {
        ...
        $list['order_total']=CrmOrderModel::where('paid', 1)->where('institu_leader',$this->instituPId)->where('pay_time', '>', $now_month)->sum('pay_price');
        //本月订单新增
        $list['order_count']= CrmOrderModel::where('paid', 1)->where('institu_leader',$this->instituPId)->where('add_time', '>', $now_month)->count();
        //医生总数
        $list['doctor']=CrmUserModel::where(['institu_leader'=>$this->instituPId,'type'=>1])->count();
        //患者总数
        $list['patient']=CrmUserModel::where(['institu_leader'=>$this->instituPId,'type'=>0])->count();
        $info['doctor']['num']=$list['doctor'];
        $info['doctor']['now_month_num']=CrmUserModel::where(['institu_leader'=>$this->instituPId,'type'=>1])->where('createtime', '>', $now_month)->count();
        $info['patient']['num']=$list['patient'];
        $info['patient']['now_month_num']=CrmUserModel::where(['institu_leader'=>$this->instituPId,'type'=>0])->where('createtime', '>', $now_month)->count();
        $this->assign([
    ...
    public function main()
    {
        ...
        //新粉丝->日
        $now_day_user = CrmUserModel::where('add_time', '>', $now_day)->count();
        $pre_day_user = CrmUserModel::where('add_time', '>', $pre_day)->where('add_time', '<', $now_day)->count();
        ...
        //新粉丝->月
        $now_month_user = CrmUserModel::where('add_time', '>', $now_month)->count();
        $pre_month_user = CrmUserModel::where('add_time', '>', $pre_month)->where('add_time', '<', $now_month)->count();
        $first_line['month'] = [
