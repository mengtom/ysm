{include file="public/header"}
	<title>订单管理</title>
	<div class="h-100 w-100 mi-order" id="app">
		<!--头部-->
		{include file="public/header_navigate"}
		<!--头部-->
		<div class="w-1200 mt-80 mb-10 tip  text-666 size-14">
			<a class="text-57">首页 /</a> <span class="text-c01f5e">订单查看 </span>
		</div>
		...
		<div style="height: 34px;" class="w-1200">
			...
			<div class="h-100 w-100px text-center ellipsis-1 cursor tab-item relative " :class="{'tab-item-active':where.status===item.value}" @click="where.status = item.value" type="button" v-for="item in orderStatus">{{item.name}}<span v-if="item.count!=undefined" >({{item.count}})</span></div>
				...
				<div class="h-100 w-100px text-center ellipsis-1 cursor tab-item relative"><a href="">已关闭(00)</a></div>
				<div class="h-100 w-100px text-center ellipsis-1 cursor tab-item relative"><a href="">退款申请(00)</a></div>
				<div class="h-100 w-100px text-center ellipsis-1 cursor tab-item relative"><a href="">已退款(00)</a></div> -->
			</div>
		</div>
		...
		<div class="w-1200 relative" style="border: 1px solid #d9d9d9;z-index: 1">
			<div class="w-100 com-search" style="border-bottom: 2px solid #f0f0f0;margin-top: 0;">
				<div class="keyword w-100 bg-fff  h-auto">
					<form class="w-100 h-100 layui-form  d-flex align-items-start justify-content-between" action="" method="get">
						...
						<div class="h-100 pl-20 d-flex align-items-center justify-content-start flex-column mt-20 mb-20">
							...
							<div class="w-100 d-flex align-items-center justify-content-start">
								...
								<div class="h-100 d-flex align-items-center justify-content-start ml-20">
									<span style="width: 40px;">医生:</span>
									<input class="ivu-input w-150px" placeholder="请输入" type="text" v-model="where.doctor">
								</div>
							...
							<!-- <div class="w-100 d-flex align-items-center justify-content-start mt-20">
								<div class="h-100 d-flex align-items-center justify-content-start">
									<span style="width: 90px;">下单时间:</span>
									<div class="ivu-select  ivu-select-single ivu-select-default relative">
										<input class="ivu-input" type="text" placeholder="开始时间->结束时间" style="width: 240px;" id="test13" v-model="where.ordersn">
									</div>
								</div>
							</div> -->
						</div>
			...
			<div class="w-100 label-list bg-fff">
				<div class="w-100 list-title d-flex align-items-center justify-content-between mb-20 pt-20">
					...
					<div>
						<a href="" target="_blank" class="ivu-btn ivu-btn-default mr-20">
				...
				<div class="w-100 table ">
					<div class="t-table" style="width: 1160px;margin: auto">
						...
						<div class="w-100 bg-f6 d-flex align-items-center justify-content-start text-57" style="height: 32px;margin: auto;border: 1px solid #d9d9d9">
							<div class="text-center" style="width: 414px;"> 订单信息</div>
							<div class="w-100px text-center">开具医生姓名</div>
							<div class="w-100px text-center">单剂价格(元)</div>
							<div class="w-100px text-center">数量</div>
							<div class="w-100px text-center">总价(元)</div>
							<div class="w-100px text-center" style="width: 128px;">收货人信息</div>
							<div class="w-100px text-center">支付方式</div>
							<div class="w-100px text-center">订单状态</div>
						</div>
						...
						<div v-for="i in items" class="w-100 d-flex align-items-center justify-content-start flex-column t-tabl-top">
							...
							<div class="w-100 d-flex align-items-center justify-content-start t-tabl-top-centen">
								<div class="h-100  d-flex align-items-center pl-20 " style="width: 414px;"> 处方名称:{{i.order_name}}</div>
								<div class="h-100 w-100px d-flex align-items-center justify-content-center  bl">{{i.nickname}}</div>
								<div class="h-100 w-100px d-flex align-items-center justify-content-center  bl">{{i.price}}</div>
								<div class="h-100 w-100px d-flex align-items-center justify-content-center flex-column  bl">
								...
								<div class="h-100 w-100px d-flex align-items-center justify-content-center bl flex-column" style="width: 128px;">
									<span class="w-100 text-center text-27">{{i.real_name}}</span>
									<span class="w-100 text-center text-27">{{i.user_phone}}</span>
								</div>
								<div class="h-100 w-100px d-flex align-items-center justify-content-center bl">{{i.pay_type_name}}</div>
								<div class="h-100 w-100px d-flex align-items-center justify-content-center  flex-column bl">
							...
							<div class="w-100 d-flex align-items-center justify-content-center" style="min-height: 42px;">
								<span class="text-c01f5e cursor" data-id="1" onclick="unfold(this)"><i class="icon iconfont iconxiangxia_hei"></i> 详情</span>
							</div>
						</div>
...
<script type="text/javascript">
	...
	var app   = new Vue({
		...
		methods: {
	        ...
	        excel:function () {
	            this.where.excel=1;
	            location.href=layList.U({m:'institu',c:'crm.crm_order',a:'order_list',q:this.where});
	        },
			// 页码改变的回调，返回改变后的页码
			pagechange(e) {
				this.pagenum = e;
	...
	$('.shipments').click(function () {
		// $('.shipments-modal').show();
		var url=layList.U({m:'institu',c:'crm.crm_order',a:'order_goods',q:{id:data.id}});
	...
	function shipments(title,id){
		var url=layList.U({m:'institu',c:'crm.crm_order',a:'order_goods',q:{id:id}});
