...
namespace app\apip\controller;


use app\http\validates\user\RegisterValidates;
use app\models\crm\SystemMenus;
use app\models\crm\CrmUsers as user;
use app\models\crm\Platform;
use app\models\crm\UserToken;
use app\models\crm\PlatformToken;
use app\models\crm\WechatUser;
use app\Request;
use yesai\jobs\TestJob;
use yesai\services\CacheService;
use yesai\services\SMSService;
...
class AuthController
{
    ...
    {
        $Platform = Platform::where('platform_appid', $request->param('appid'))->find();
        if($Platform) {
            if ($Platform->platform_app_secret !== $request->param('secret'))
                return app('json')->fail('权限验证失败');
        }else{
            return app('json')->fail('没有权限');
        }
        if (!$Platform['status'])
            return app('json')->fail('已被禁止，请联系管理员');
        $token = PlatformToken::createToken($Platform, 'platform');
        if ($token) {
            event('PlatformLogin', [$Platform, $token->token,'apip']);
            return app('json')->success('登录成功', ['access_token' => $token->token, 'expires_time' => $token->expires_time]);
        } else
            return app('json')->fail('登录失败');
    }
    /**
