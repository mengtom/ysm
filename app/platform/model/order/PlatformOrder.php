 ...
 * @day: 2020/07/23
 */
namespace app\platform\model\order;
use app\platform\model\ump\StoreCouponUser;
use app\platform\model\wechat\WechatUser;
...
class PlatformOrder extends BaseModel
{
    ...
    {
        $data['wz'] = self::statusByWhere(0, new self())->where(['is_system_del' => 0])->count();//未付款
        $data['wf'] = self::statusByWhere(1, new self())->where(['is_system_del' => 0, 'shipping_type' => 1])->count();//待发货
        $data['ds'] = self::statusByWhere(2, new self())->where(['is_system_del' => 0, 'shipping_type' => 1])->count();//待收
        $data['dp'] = self::statusByWhere(3, new self())->where(['is_system_del' => 0])->count();//待点评
        $data['jy'] = self::statusByWhere(4, new self())->where(['is_system_del' => 0])->count();//完成
        $data['tk'] = self::statusByWhere(-1, new self())->where(['is_system_del' => 0])->count();//退款申请
        $data['yt'] = self::statusByWhere(-2, new self())->where(['is_system_del' => 0])->count();//已退
        $data['del'] = self::statusByWhere(-4, new self())->where(['is_system_del' => 0])->count();//关闭
    ...
    {
        $model = self::getOrderWhere($where, self::alias('a')->join('crm_users r', 'r.user_id=a.user_id', 'LEFT'), 'a.', 'r')->field('a.*,r.nickname,r.account as phone,r.spread_uid');
        if ($where['order'] != '') {
        ...
        foreach ($data as &$item) {
            // $_info = Db::name('crm_order_cart_info')->where('oid', $item['id'])->field('cart_info')->select();
            ...
            // }
            // $item['_info'] = $_info;
            $micro=Db::name('crm_order_microchip')->where('order_id', $item['id'])->field('micro_code,micro_name,micro_price,single_price,num,total_dose')->select();
            $item['_micro']=$micro;
            $item['micro_num']=count($micro);
            $item['micro_total']=Db::name('crm_order_microchip')->where('order_id', $item['id'])->field('num')->count();
...
HTML;
        ...
        $count = self::getOrderWhere($where, self::alias('a')->join('crm_users r', 'r.user_id=a.user_id', 'LEFT'), 'a.', 'r')->count();
...
//                    $model = $model->where($aler.'combination_id',">",0)->where($aler.'pink_id',">",0);
        ...
        if(isset($where['ordersn']) && $where['ordersn']){
            $model = $model->where($aler . 'order_id', 'LIKE', "%$where[ordersn]%");
        }
        if (isset($where['nickname']) && $where['nickname'] != '') {
            $model = $model->where($aler . 'order_id|' . $aler . 'real_name|' . $aler . 'user_phone' . ($join ? '|' . $join . '.nickname|' . $join . '.user_id|' . $join . '.account' : ''), 'LIKE', "%$where[nickname]%");
        }
        ...
        if(isset($where['time']) && $where['time'] !=''){
            ...
            $model=$model->whereBetween($aler . 'createtime',[strtotime($startTime),strtotime($endTime)]);
        }
        if(isset($where['platform']) && $where['platform']){
             $model = $model->where($join . '.platform_name', $where['platform']);
        }
        if(isset($where['institu']) && $where['institu']){
             $model = $model->where($join . '.institu_name', $where['institu']);
        }
        return $model;
    }
