...
namespace app\platform\model\ts;

// use app\platform\model\ump\StoreBargain;
// use app\platform\model\ump\StoreCombination;
// use app\platform\model\ump\StoreSeckill;
...
// use app\platform\model\order\StoreOrder;
// use app\platform\model\system\SystemConfig;
use app\platform\model\ts\TsMicrochip as MicrochipModel;
// use app\platform\model\crm\CrmPlatformTs as tsModel;
/**
 * 配方管理 model
 * Class StoreProduct
 * @package app\platform\model\ts
 */
class Ts extends BaseModel
{
    ...
    public static function setData($type){
        switch ((int)$type){
            case 1:
                $data = ['ts.is_status'=>1,'ts.is_del'=>0];
                break;
            case 0:
                $data = ['ts.is_status'=>0,'ts.is_del'=>0];
                break;
            // case 3:
            //     $data = ['p.is_del'=>0];
            //     break;
            // case 4:
            //     $data = ['p.is_status'=>1,'p.is_del'=>0,'pav.stock|p.stock'=>0];
            //     break;
            // case 5:
            //     $data = ['p.is_status'=>1,'p.is_del'=>0];
            //     break;
            // case 6:
            //     $data = ['p.is_del'=>1];
            //     break;
    ...
    public static function getModelObject($where=[]){
        $model=new self();
        $model=$model->alias('pi')->field('pi.*');
        if(!empty($where)){
            $model=$model->group('pi.id')->where(['pi.is_del'=>0]);
            if(isset($where['type'])){
                switch($where['type']){
                    case 1:
                        $model = $model->where('pi.type','=',0);
                    break;
                    case 2:
                        $model = $model->where('pi.type','=',4);
                    break;
                    default:
                        $model = $model->where('pi.type','In','0,4');
                    break;
                }
            }
            if(isset($where['cate_id']) && trim($where['cate_id'])!=''){
                $model=$model->where(self::getPidSql($where['cate_id'],'i.cate_id'));
            }
            if(isset($where['keyword']) && $where['keyword']!=''){
                $model = $model->where('pi.code|pi.name_zn|pi.name_en','LIKE',"%{$where['keyword']}%");
            }
            if(isset($where['status']) && trim($where['status'])!=''){
                $model=$model->where('pi.is_status',$where['status']);
            }
            if(isset($where['platform_id']) && $where['platform_id'] !=''){
                $ts_id = DB::name('crm_platform_ts')->where(['platform_id'=>$where['platform_id'],'status'=>1])->column('ts_id');
                if($ts_id){
                     $model = $model->where("pi.id In (".implode(',',$ts_id).") or pi.platform_id = ".$where['platform_id']);
                }else{
                     $model = $model->where("pi.platform_id = ".$where['platform_id']);
                }
     ...
     *
     */
    public static function TsList($where){
        $model=self::getModelObject($where)->field('*,FROM_UNIXTIME(add_time,"%Y-%c-%d %h:%i") as add_time');
        if($where['excel']==0 && ($where['limit'] > 0 && $where['page'] > 0)) $model=$model->page((int)$where['page'],(int)$where['limit']);
        ...
        if($where['excel']==1){
            ...
            PHPExcelService::setExcelHeader(['配方名称','配方简介','配方分类','价格','库存','销量','点赞人数','收藏人数'])
                ->setExcelTile('配方导出','配方信息'.time(),' 生成时间：'.date('Y-m-d H:i:s',time()))
    ...
    public static function getMaxList($where){
        ...
        $model=StoreOrder::alias('a')->join('StoreOrderCartInfo c','a.id=c.oid')->join('ts_product b','b.id=c.product_id');
        $list=self::getModelTime($where,$model,'a.add_time')->group('c.product_id')->order('p_count desc')->limit(10)
            ->field(['count(c.product_id) as p_count','b.ts_name','sum(b.price) as sum_price'])->select();
        ...
        foreach ($list as $key=>&$item){
            ...
            $item['ts_name']=self::getSubstrUTf8($item['ts_name']);
        }
    ...
    public static function ProfityTop10($where){
        ...
        $model=StoreOrder::alias('a')
            ...
            ->join('ts_product b','b.id=c.product_id')
            ->where('b.is_show',1)
            ->where('b.is_del',0);
        $list=self::getModelTime($where,$model,'a.add_time')->group('c.product_id')->order('profity desc')->limit(10)
            ->field(['count(c.product_id) as p_count','b.ts_name','sum(b.price) as sum_price','(b.price-b.cost) as profity'])
            ->select();
        if(count($list)) $list=$list->toArray();
        $maxList=[];
        $sum_count=0;
        ...
        foreach ($list as $key=>&$item){
            ...
            $item['class']=isset($classs[$key]) ?$classs[$key]:( isset($classs[$key-count($classs)]) ? $classs[$key-count($classs)]:'');
            $item['ts_name']=self::getSubstrUTf8($item['ts_name'],30);
        }
        $maxList['sum_count']=$sum_count;
        $maxList['sum_price']=$sum_price;
        $maxList['list']=$list;
    ...
    public static function getLackList($where){
        $replenishment_num = SystemConfig::getConfigValue('replenishment_num');
        $replenishment_num = $replenishment_num > 0 ? $replenishment_num : 20;
        $list=self::where('stock','<',$replenishment_num)->field(['id','ts_name','stock','price'])->page((int)$where['page'],(int)$where['limit'])->order('stock asc')->select();
        if(count($list)) $list=$list->toArray();
    ...
    public static function getnegativelist($where){
        $list=self::alias('s')->join('StoreProductReply r','s.id=r.product_id')
            ->field('s.id,s.ts_name,s.price,count(r.product_id) as count')
    ...
    public static function TuiProductList(){
        ...
        foreach ($perd as &$item){
            $item['ts_name']=self::where(['id'=>$item['id']])->value('ts_name');
    ...
    public static function getTierList($model = null)
    {
        ...
        return $model->field('id,ts_name')->where('is_del',0)->select()->toArray();
    }
    /**
     * 设置查询条件
     * @param array $where
    ...
    public static function setWhere($where){
        ...
        $model=self::getModelTime($time, Db::name('ts_cart')->alias('a')->join('ts_product b','a.product_id=b.id'),'a.add_time');
        if(isset($where['title']) && $where['title']!=''){
            $model=$model->where('b.ts_name|b.id','like',"%$where[title]%");
        }
        return $model;
    }
    /**
    ...
    public static function getSaleslists($where){
        $data=self::setWhere($where)->where('a.is_pay',1)
            ->group('a.product_id')
            ->field(['sum(a.cart_num) as num_product','b.ts_name','b.image','b.price','b.id'])
    ...
    public static function SaveProductExport($where){
        ...
        PHPExcelService::setExcelHeader(['商品编号','商品名称','商品售价','销售额','销量'])
            ->setExcelTile('配方销量排行','配方销量排行',' 生成时间：'.date('Y-m-d H:i:s',time()))
            ->setExcelContent($export)
            ->ExcelSave();
    }
    /*
    ...
    public static function getProductBadgeList($id,$where){
        ...
        return [
            ...
            [
                'name'=>'点赞次数',
                'field'=>'个',
                'count'=>Db::name('ts_product_relation')->where('product_id',$id)->where('type','like')->count(),
                'background_color'=>'layui-bg-blue',
            ],
            [
                'name'=>'销售总额',
     ...
     * @return mixed
     */
    public static function getProductField($id,$field = 'ts_name'){
     ...
     * @return array
     */
    public static function TsMicorchipList($where){
        $model=self::getModelObject($where);
        if($where['excel']==0 && ($where['limit'] > 0 && $where['page'] > 0)) $model=$model->page((int)$where['page'],(int)$where['limit']);
        $data=($data=$model->select()) && count($data) ? $data->toArray():[];
        $currency=$where['currency'] == '人民币'? true:false;
        foreach ($data as $k=> &$item){
            switch($item['type']){
                case 1:
                    $item['type_name']='独立医生配方';
                    break;
                case 2:
                    $item['type_name']='医生机构配方';
                    break;
                case 3:
                    $item['type_name']='机构配方';
                    break;
            ...
            $item['microchip']=MicrochipModel::where("ts_id",$item['id'])->select()->toArray();
        }unset($item);
        $count=self::getModelObject($where)->count();
        return compact('count','data');
    }
     ...
     * @return mixed
     */
    public static function checkTsExist($field,$value,$id){
        $info=self::where($field,$value)->find();
        if($info && $info['id'] != $id)
            return $info;
        else
            return false;
