...
namespace app\platform\model\microchip;

use app\platform\model\ump\MicrochipBargain;
use app\platform\model\ump\MicrochipCombination;
use app\platform\model\ump\MicrochipSeckill;
...
use app\platform\model\order\MicrochipOrder;
use app\platform\model\system\SystemConfig;

/**
 * 产品管理 model
 * Class MicrochipProduct
 * @package app\platform\model\microchip
 */
class MicrochipProduct extends BaseModel
{
    ...
    public static function getModelObject($where=[]){
        $model=new self();
        //$model=$model->alias('p')->join('MicrochipProductAttrValue pav','p.id=pav.product_id','LEFT');
        if(!empty($where)){
            ...
            //         $model = $model->where($data)->where('p.stock','<=',$microchip_stock);
            //     } else {
