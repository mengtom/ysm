...
// use app\platform\model\crm\CrmCategory as CategoryModel;
use app\platform\model\order\CrmOrder;
use app\platform\model\system\SystemConfig;

/**
 * 平台管理 model
 * Class CrmPlatform
 * @package app\platform\model\crm
 */
class CrmInstitution extends BaseModel
{
     ...
     * @var string
     */
    protected $name = 'crm_institution';
    use ModelTrait;
    /**
     * 获取连表查询条件
     * @param $type
    ...
    public static function getModelObject($where=[]){
        ...
        if(!empty($where)){
            ...
            if(isset($where['category']) && in_array($where['category'],[2,3]) && $where['category'] !=''){
     ...
     *
     */
    public static function InstituList($where,$fieldout=[]){
     ...
     */
    public static function addInstitution($data){
        if(self::get(['refer_mobile'=>$data['refer_mobile']])) return self::setErrorInfo('联系方式不能重复');
     ...
     */
    public static function getInstituInfo($where){
