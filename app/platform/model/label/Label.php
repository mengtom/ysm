 ...
 * @day: 2020/6/10
 */
namespace app\platform\model\label;
 ...
 * 标签标签model
 * Class LabelCategory
 * @package app\platform\model\wechat
 */
class Label extends BaseModel
{
    ...
    public static function getAll($where = array()){
        ...
        return self::page($model,function($item){
            // if(!$item['mer_id']) $item['platform_name'] = '总后台管理员---》'.SystemAdmin::where('id',$item['platform_id'])->value('real_name');
            // else $item['platform_name'] = Merchant::where('id',$item['mer_id'])->value('mer_name').'---》'.MerchantAdmin::where('id',$item['platform_id'])->value('real_name');
