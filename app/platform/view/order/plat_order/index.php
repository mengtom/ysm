{include file="public/header"}
	...
	<div class="h-100 w-100 order-order-p" id="app">
		<!--头部-->
		{include file="public/header_navigate"}
		<!--头部-->
		<div class="w-1200 mt-80 mb-10  text-666 size-14">
			<a class="text-57">首页 /</a> <span class="text-c01f5e">查看订单 </span>
		</div>
						...
						<div class="w-100 d-flex align-items-center justify-content-start flex-column t-tabl-top" v-for="i in items">
							...
							<div class="w-100 d-flex align-items-center justify-content-start t-tabl-top-centen">
								...
								<div class="h-100 d-flex align-items-center justify-content-center flex-column w-90px bl">
									...
									<button type="submit"  class="ivu-btn ivu-btn-primary ivu-btn-small mt-8 shipments" style="width: 72px;" onclick="shipments('发货',{{i.id}})" >发货</button>
...
<script type="text/javascript">
	...
	var app   = new Vue({
		...
		methods: {
	        ...
	        getBadge:function() {
	            var that=this;
	            layList.basePost(layList.Url({c:'order.plat_order',a:'getBadge'}),this.where,function (rem) {
	                that.badge=rem.data;
	            });
	        },
	        search:function () {
	        ...
	        excel:function () {
	            this.where.excel=1;
	            location.href=layList.U({c:'order.plat_order',a:'order_list',q:this.where});
	        },
			// 页码改变的回调，返回改变后的页码
			pagechange(e) {
				this.pagenum = e;
