	...
	<div class="h-100 w-100 cmr-wpindex" id="app">
		...
		<div class="w-1200 label-list mt-10 mb-30 bg-fff">
			<div class="w-100 list-title d-flex align-items-center justify-content-between mb-20 pt-20">
				<span class="size-16  ml-20"><span class="fw">平台微片选择</span> <span class="size-14">（上海XXXX科技有限公司）</span> </span>
				<div>
			...
			<div class="w-100 table ">
				<form action="" class="">
					...
					<script type="text/html" id="status" lay-filter="status">
						<input type='checkbox' lay-skin='primary' lay-filter='checkboxIsSelected' {{ d.status == 1 ? 'checked' : '' }} class='checkboxIsSelected' value='1'>
					</script>
				</form>
			</div>
			<!--page 分页-->
...
<script type="text/javascript">
    ...
    layList.tableList('list',"{:Url('platform_microList',array('id'=>$platform.id))}",function (){
        ...
        join=[
            ...
            {field: 'dose_range', title: '剂量范围',templet:'#dose_range',align:'center', event:'dose_max',width:'8%'},
            {field: 'dose', title: '单次增量',templet:'#dose',align:'center', event:'dose',width:'8%'},
            {field: 'currency', title: '结算货币',templet:'#currency',align:'center', event:'currency',width:'8%'},
            {field: 'price', title: '成本价',templet:'#price',align:'center', event:'price',width:'8%'},
            {field: 'sell_price', title: '建议售价',templet:'#sell_price',align:'center', event:'sell_price',width:'8%'},
            {field: 'platform_price', title: '平台结算价',templet:'#platform_price',align:'center',edit:'platform_price',width:'8%'},
            {type:'checkbox'},
        ];
        return join;
    })
    ...
    layList.edit(function (obj) {
        ...
        switch (obj.field) {
            case 'platform_price':
                action.set_product('platform_price',id,value,pid);
                break;
        }
    });
    ...
    //自定义方法
    var action={
        set_product:function(field,id,value,pid){
            layList.baseGet(layList.Url({m:'admin',c:'crm.crm_platform',a:'edit_platform_price',q:{field:field,id:id,value:value,pid:pid}}),function (res) {
                layList.msg(res.msg);
            });
        },
        save_micro:function(){
