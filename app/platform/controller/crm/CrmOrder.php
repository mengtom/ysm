...
use app\platform\model\system\Express;
use yesai\repositories\OrderRepository;
use yesai\services\ExpressService;
...
use app\platform\model\crm\CrmOrderStatus;
use app\platform\model\ump\StorePink;
use app\platform\model\user\User;
use app\platform\model\user\UserBill;
use yesai\basic\BaseModel;
use yesai\services\CacheService;
use yesai\services\UtilService as Util;
use yesai\services\JsonService as Json;
use think\facade\Db;
use think\facade\Route as Url;
use app\platform\model\crm\CrmOrder as CrmOrderModel;
/**
 * 订单管理控制器 同一个订单表放在一个控制器
 * Class StoreOrder
 * @package app\platform\controller\store
 */
class CrmOrder extends AuthController
{
    /**
     * @return mixed
     */
    public function index()
    {
        ...
        $this->assign([
            ...
            'orderCount'=>CrmOrderModel::userOrderCount($type,3,$id),
            // 'payTypeCount'=>CrmOrderModel::payTypeCount(),
        ]);
        return $this->fetch();
    }
    /**
    ...
    //         ['order','']
    //     ]);
    //     return JsonService::successful(CrmOrderModel::getBadge($where));
    // }
    /**
     * 获取订单列表
     * return json
    ...
    public function order_list(){
        ...
        $where['platform_id']=$this->platformPId;
        return JsonService::successlayui(CrmOrderModel::OrderList($where));
    }
    /**
     * 核销码核销
    ...
    //         CrmOrderModel::beginTrans();
    //         $orderInfo = CrmOrderModel::where('verify_code',$verify_code)->where('paid',1)->where('refund_status',0)->find();
    ...
    //             CrmOrderStatus::setStatus($orderInfo->id,'take_delivery','已核销');
    //             //发送短信
    //             event('ShortMssageSend',[$orderInfo['order_id'],'Receiving']);
    //             CrmOrderModel::commitTrans();
    //             return JsonService::successful('核销成功！');
    //         }else {
    //             CrmOrderModel::rollbackTrans();
    ...
    public function orderchart(){
        ...
        $orderCount = [
            urlencode('未支付')=>CrmOrderModel::getOrderWhere($where,CrmOrderModel::statusByWhere(0))->count(),
            urlencode('未发货')=>CrmOrderModel::getOrderWhere($where,CrmOrderModel::statusByWhere(1))->count(),
            urlencode('待收货')=>CrmOrderModel::getOrderWhere($where,CrmOrderModel::statusByWhere(2))->count(),
            urlencode('交易完成')=>CrmOrderModel::getOrderWhere($where,CrmOrderModel::statusByWhere(4))->count(),
            urlencode('退款中')=>CrmOrderModel::getOrderWhere($where,CrmOrderModel::statusByWhere(-1))->count(),
            urlencode('已退款')=>CrmOrderModel::getOrderWhere($where,CrmOrderModel::statusByWhere(-2))->count()
        ];
        $model = CrmOrderModel::getOrderWhere($where,new CrmOrderModel())->field('sum(total_num) total_num,count(*) count,sum(total_price) total_price,sum(refund_price) refund_price,from_unixtime(add_time,\'%Y-%m-%d\') add_time')
        ...
        $this->assign(CrmOrderModel::systemPage($where,$this->adminId));
        $this->assign('price',CrmOrderModel::getOrderPrice($where));
...
//     }
    ...
    public function update($id)
    {
        ...
        $data['order_id'] = CrmOrderModel::changeOrderId($data['order_id']);
        CrmOrderModel::edit($data,$id);
        event('StoreProductOrderEditAfter',[$data,$id]);
        CrmOrderStatus::setStatus($id,'order_edit','修改商品总价为：'.$data['total_price'].' 实际支付金额'.$data['pay_price']);
    ...
    //     if(CrmOrderModel::where('is_del',0)->where('id','in',$ids)->count()) return JsonService::fail('您选择的的订单存在用户未删除的订单，无法删除用户未删除的订单');
    //     $res=CrmOrderModel::where('id','in',$ids)->update(['is_system_del'=>1]);
    //     if($res)
    ...
    public function update_delivery($id = 0){
        ...
        switch ((int)$data['type']){
            case 1:
                ...
                if(!$data['delivery_name']) return $this->failed('请选择快递公司');
                if(!$data['delivery_id']) return $this->failed('请输入快递单号');
                $data['status'] = 1;
                CrmOrderModel::edit($data,$id);
                // event('CrmProductOrderDeliveryGoodsAfter',[$data,$id]);//推送消息
                CrmOrderStatus::setStatus($id,'delivery_goods','已发货 快递公司：'.$data['delivery_name'].' 快递单号：'.$data['delivery_id']);
                break;
            case 2:
                //送货
                ...
                CrmOrderStatus::setStatus($id,'delivery','已配送 发货人：'.$data['delivery_name'].' 发货人电话：'.$data['delivery_id']);
                break;
            case 3:
                //虚拟发货
                ...
                CrmOrderModel::edit($data,$id);
                event('StoreProductOrderDeliveryAfter',[$data,$id]);
                CrmOrderStatus::setStatus($id,'delivery_fictitious','已虚拟发货');
                break;
            default:
                return $this->failed('暂时不支持其他发货类型');
                break;
        }
        //短信发送
        //event('ShortMssageSend',[CrmOrderModel::where('id',$id)->value('order_id'),'Deliver']);
        return $this->successful('发货成功!');
    }
    ...
    public function take_delivery($id){
        ...
        CrmOrderModel::beginTrans();
        try{
            if(!CrmOrderModel::edit($data,$id)) {
                CrmOrderModel::rollbackTrans();
                return Json::fail(CrmOrderModel::getErrorInfo('收货失败,请稍候再试!'));
            }else{
                OrderRepository::storeProductOrderTakeDeliveryAdmin($order, $id);
                CrmOrderStatus::setStatus($id,'take_delivery','已收货');
                CrmOrderModel::commitTrans();
                //发送短信
    ...
    public function updateRefundY($id){
        ...
        $resEdit = CrmOrderModel::edit($data,$id);
        $res = true;
        if($resEdit){
            $data['type'] = $type;
            ...
            CrmOrderStatus::setStatus($id,'refund_price','退款给用户'.$refund_price.'元');
            BaseModel::commitTrans();
            return Json::successful('修改成功!');
        }else{
            CrmOrderStatus::setStatus($id,'refund_price','退款给用户'.$refund_price.'元失败');
            return Json::fail('修改失败!');
        }
    }
    public function order_info($oid = '')
    {
        if(!$oid || !($orderInfo = CrmOrderModel::get($oid)))
    ...
    {
        if(!$oid || !($order = CrmOrderModel::get($oid)))
            return $this->failed('订单不存在!');
    ...
    public function updateDistribution($id){
        ...
        CrmOrderStatus::setStatus($id,'distribution','修改发货信息为'.$data['delivery_name'].'号'.$data['delivery_id']);
        return Json::successful('修改成功!');
    }
    /**
     * 修改退款状态
    ...
    public function updateRefundN($id){
        ...
        CrmOrderStatus::setStatus($id,'refund_n','不退款原因:'.$data['refund_reason']);
        return Json::successful('修改成功!');
    }
    /**
     * 立即支付
    ...
    public function offline($id){
        $res = CrmOrderModel::updateOffline($id);
        if($res){
            event('StoreProductOrderOffline',[$id]);
            CrmOrderStatus::setStatus($id,'offline','线下付款');
            return Json::successful('修改成功!');
        }else{
            return Json::fail(CrmOrderModel::getErrorInfo('修改失败!'));
        }
    }
    /**
     * 修改积分和金额
    ...
    public function updateIntegralBack($id){
        ...
        CrmOrderModel::edit($data,$id);
        CrmOrderStatus::setStatus($id,'integral_back','商品退积分：'.$data['back_integral']);
    ...
    public function order_status($oid){
       ...
       $this->assign(CrmOrderStatus::systemPage($oid));
       return $this->fetch();
    }
    ...
    public function order_details(int $id){
        if(!$id) return $this->failed('数据不存在');
        $order = CrmOrderModel::getOrderDetails($id);
