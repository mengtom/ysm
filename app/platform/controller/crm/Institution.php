...
class Institution extends AuthController
{
    ...
    * */
    ...
    public function save()
    {
        ...
        if(!$res){
            BaseModel::rollbackTrans();
            return $this->failed(InstituModel::getErrorInfo());
        }
    ...
    public function order(int $id){
        ...
        $this->assign([
            ...
            'orderCount'=>$order::userOrderCount($type,2,$id),
            'type'=>$type,
        ]);
        $this->assign(compact('id','institu'));
        return $this->fetch();
    }
    ...
    public function order_details(int $id){
        ...
        $institu = InstituModel::getInstituInfo(['id'=>$i_id]);
        if(!$institu) return $this->failed('数据不存在!');
        $CrmOrder=new \app\admin\model\crm\CrmOrder;
        $order = $CrmOrder::getOrderDetails($id);
        $this->assign(compact('id','i_id','order'));
        return $this->fetch();
    }
}
