...
class CrmDoctor extends AuthController
{
    ...
    public function save()
    {
        ...
        $docVa=$DoctorValidate->scene('absic')->check($data);
        if(!$docVa){
        ...
        $data['currency'] = $this->platformInfo['currency'] ? '美元':'人民币';
    ...
    public function update($id)
    {
        ...
        if($data['group'] <=0){
            $res=CrmUsers::update(['group'=>0,'group_name'=>'','group_commission'=>$doctor['group_commission'],'group_discount'=>$doctor['group_discount']],['user_id'=>$doctor['uid']]);
   ...
   /**
    ...
    public function order(int $id){
        ...
        $doctor = DoctorModel::getDoctorInfo(['id'=>$id]);
        if(!$doctor) return $this->failed('数据不存在!');
        $order=new \app\platform\model\crm\CrmOrder;
        $type=$this->request->param('type',0);
        $this->assign([
            'year'=>getMonth(),
            'real_name'=>$this->request->get('real_name',''),
            'status'=>$this->request->param('status',''),
            'orderCount'=>$order::userOrderCount($type,1,$id),
            'type'=>$type,
        ]);
        $this->assign(compact('id','doctor'));
        return $this->fetch();
    }
    public function order_list(){
        ...
        return Json::successlayui(\app\platform\model\crm\CrmOrder::OrderList($where));
    }
    public function order_details(int $id){
        if(!$id) return $this->failed('数据不存在');
        $d_id=$this->request->param('d_id',0);
        $doctor = DoctorModel::getDoctorInfo(['id'=>$d_id]);
        if(!$doctor) return $this->failed('数据不存在!');
        $CrmOrder=new \app\admin\model\crm\CrmOrder;
        $order = $CrmOrder::getOrderDetails($id);
        $this->assign(compact('id','d_id','order'));
        return $this->fetch();
    }
}
