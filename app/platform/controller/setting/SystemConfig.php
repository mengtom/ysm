<?php
namespace app\platform\controller\setting;
...
use app\platform\model\crm\SystemConfig as ConfigModel;
/**
 *  配置列表控制器
 * Class SystemConfig
 * @package app\platform\controller\system
 */
class SystemConfig extends AuthController
{
   ...
   public function index(){
       ...
       $list = ConfigModel::getAll($tab_id,$this->platformPId);
       $platform=$formbuider=[];
       if($type==3){//其它分类
          $config_tab = null;
       }else{
           ...
           foreach ($config_tab as $kk=>$vv){
               $arr = ConfigModel::getAll($vv['value'],$this->platformPId)->toArray();
               // if(empty($arr)){
               //     unset($config_tab[$kk]);
               // }
           }
           ...
           switch ($data['type']){
               ...
               case 'upload'://文件上传
                   switch ($data['upload_type']){
                       case 1:
                           $data['value'] = json_decode($data['value'],true)?:'';
                           $formbuider[] = Form::frameImageOne($data['menu_name'],$data['info'],Url::buildUrl('platform/widget.images/index',array('fodder'=>$data['menu_name'])),$data['value'])->icon('image')->width('70%')->height('500px')->info($data['desc'])->col(13);
                           break;
                       case 2:
                           $data['value'] = json_decode($data['value'],true)?:[];
                           $formbuider[] = Form::frameImages($data['menu_name'],$data['info'],Url::buildUrl('platform/widget.images/index',array('fodder'=>$data['menu_name'])),$data['value'])->maxLength(5)->icon('image')->width('70%')->height('500px')->info($data['desc'])->col(13);
                           break;
                       case 3:
                           $data['value'] = json_decode($data['value'],true);
      ...
      if($type == 0){
          $platform=[
          'name'=>$this->platformInfo['p_name'],
          'referral_code'=>$this->platformInfo['referral_code'],
          'appid'=>$this->platformInfo['platform_appid'],
          'secret'=>$this->platformInfo['platform_app_secret'],
          ];
       }
       $form = Form::make_post_form('编辑配置',$formbuider,Url::buildUrl('save_basics'));
       $url=$_SERVER['HTTP_HOST'];
       $this->assign(compact('form','platform','url'));
   ...
   public function save(){
        ...
        switch ($menu['type']){
            ...
            case 'upload':
                if($menu['upload_type'] == 1 ){
                    $menu['value'] = json_decode($menu['value'],true);
                    $formbuider[] =  Form::frameImageOne('value','图片',Url::buildUrl('platform/widget.images/index',array('fodder'=>'value')),(string)$menu['value'])->icon('image')->width('100%')->height('550px');
                }elseif ($menu['upload_type'] == 2 ){
                    $menu['value'] = json_decode($menu['value'],true)?:[];
                    $formbuider[] =  Form::frameImages('value','多图片',Url::buildUrl('platform/widget.images/index',array('fodder'=>'value')),$menu['value'])->maxLength(5)->icon('images')->width('100%')->height('550px')->spin(0);
                }else{
                    $menu['value'] = json_decode($menu['value'],true);
    ...
    public function save_basics(){
        ...
        if($request->isPost()){
            ...
            //     }
            // }
            $is_pack = isset($post['is_pack']) && $post['is_pack'] ? 1: 0;
            $is_fare = isset($post['is_fare']) && $post['is_fare'] ? 1: 0;
            $pack_num=$fare_num=0;
            if(isset($post['wechat_encode']) && $post['wechat_appid'] == ''){
                $this->failed('AppID 不能为空');
            }
            if(isset($post['wechat_encode']) && $post['wechat_appsecret'] == ''){
                $this->failed('AppSecret 不能为空');
            }
            foreach ($post as $k=>$v){
              if($k == 'is_pack' || $k == 'is_fare'){
                continue;
              }
              if($pack_num <=3){
                $is_pack=$is_pack ? 1 : 0;
                ConfigModel::where(['platform_id'=>$this->platformPId,'menu_name'=>$k])->save(['is_use' => $is_pack]);
                $pack_num++;
              }
              if($fare_num <=6){
                $is_fare=$is_fare ? 1 : 0;
                ConfigModel::where(['platform_id'=>$this->platformPId,'menu_name'=>$k])->save(['is_use' => $is_fare]);
                $fare_num++;
              }
              ConfigModel::where(['platform_id'=>$this->platformPId,'menu_name'=>$k])->save(['value' => json_encode($v)]);
            }unset($v);
            CacheService::delete(SystemConfigService::CACHE_SYSTEM);
            if($request->isAjax())
              return Json::success('修改成功');
            else
              return $this->successful('修改成功',$_SERVER['HTTP_REFERER']);
        }
    }
   /**
    * 模板表单提交
...
//    }
    public function update_token(int $length){
        if(!$length)
            Json::fail('系统异常');
        $token = random($length);
        Json::successful('获取成功',['token'=>$token]);
    }
}
