...
use app\platform\model\store\StoreProduct;
use app\platform\model\crm\SystemConfig;
use app\platform\model\crm\SystemMenus;
use app\platform\model\crm\SystemRole;
use app\platform\model\crm\CrmOrder as CrmOrderModel;//订单
use app\platform\model\user\User;
use app\platform\model\user\UserExtract as UserExtractModel;//分销
use app\platform\model\crm\CrmUsers as CrmUserModel;//用户
use app\platform\model\store\StoreProductReply as StoreProductReplyModel;//评论
use app\platform\model\store\StoreProduct as ProductModel;//产品
...
class Index extends AuthController
{
    public function index()
    {
        ...
        $this->setPlatformConfig($this->platformPId);
        // $site_logo = SystemConfig::getOneConfig('menu_name', 'site_logo')->toArray();
        //本月订单总额
        $now_month = strtotime(date('Y-m'));//本月
        //$next_month = strtotime(date('Y-m', strtotime('+1 month')));//下个月
        $list['order_total']=CrmOrderModel::where('paid', 1)->where('platform_leader',$this->platformPId)->where('pay_time', '>', $now_month)->sum('pay_price');
        //本月订单新增
        $list['order_count']= CrmOrderModel::where('add_time', '>', $now_month)->count();
        //医生总数
        $list['doctor']=CrmUserModel::where(['platform_leader'=>$this->platformPId,'type'=>1])->count();
        //患者总数
        $list['patient']=CrmUserModel::where(['platform_leader'=>$this->platformPId,'type'=>0])->count();
        //总医疗机构数量
        $info['institu']['num']=CrmUserModel::where('type','IN',[2,3])->where(['platform_leader'=>$this->platformPId])->count();
        $info['institu']['now_month_num']=CrmUserModel::where('type','IN',[2,3])->where(['platform_leader'=>$this->platformPId])->where('createtime', '>', $now_month)->count();
        $info['doctor']['num']=$list['doctor'];
        $info['doctor']['now_month_num']=CrmUserModel::where(['platform_leader'=>$this->platformPId,'type'=>1])->where('createtime', '>', $now_month)->count();
        $info['patient']['num']=$list['patient'];
        $info['patient']['now_month_num']=CrmUserModel::where(['platform_leader'=>$this->platformPId,'type'=>0])->where('createtime', '>', $now_month)->count();
        $this->assign([
    ...
    public function main()
    {
        ...
        //待发货数量
        $topData['orderDeliveryNum'] = CrmOrderModel::where('status', 0)
            ->where('paid', 1)
            ->where('refund_status', 0)
            ->count();
        //退换货订单数
        $topData['orderRefundNum'] = CrmOrderModel::where('paid', 1)
        ...
        //订单数->昨日
        $now_day_order_p = CrmOrderModel::where('paid', 1)->whereTime('pay_time', 'yesterday')->count();
        $pre_day_order_p = CrmOrderModel::where('paid', 1)->where('pay_time', '>', $pre_day)->where('pay_time', '<', $now_day)->count();
        $first_line['d_num'] = [
        ...
        //交易额->昨天
        $now_month_order_p = CrmOrderModel::where('paid', 1)->whereTime('pay_time', 'yesterday')->sum('pay_price');
        $pre_month_order_p = CrmOrderModel::where('paid', 1)->where('pay_time', '>', $beforyester_day)->where('pay_time', '<', $pre_day)->sum('pay_price');
        $first_line['d_price'] = [
        ...
        //交易额->月
        $now_month_order_p = CrmOrderModel::where('paid', 1)->whereTime('pay_time', 'month')->sum('pay_price');
        $pre_month_order_p = CrmOrderModel::where('paid', 1)->where('pay_time', '>', $pre_month)->where('pay_time', '<', $now_month)->value('sum(pay_price)');
        $first_line['m_price'] = [
            'data' => $now_month_order_p > 0 ? $now_month_order_p : 0,
            'percent' => abs($now_month_order_p - $pre_month_order_p),
            'is_plus' => $now_month_order_p - $pre_month_order_p > 0 ? 1 : ($now_month_order_p - $pre_month_order_p == 0 ? -1 : 0)
        ...
        //本月订单总数
        $now_order_info_c = CrmOrderModel::where('add_time', '>', $now_month)->count();
        $pre_order_info_c = CrmOrderModel::where('add_time', '>', $pre_month)->where('add_time', '<', $now_month)->count();
        $order_info['first'] = [
        ...
        $now_order_info_c = CrmOrderModel::where('add_time', '>', $pre_month)->where('add_time', '<', $now_month)->count();
        $pre_order_info_c = CrmOrderModel::where('add_time', '>', $second_pre_month)->where('add_time', '<', $second_now_month)->count();
        $order_info["second"] = [
            'data' => $now_order_info_c ? $now_order_info_c : 0,
            'percent' => abs($now_order_info_c - $pre_order_info_c),
            'is_plus' => $now_order_info_c - $pre_order_info_c > 0 ? 1 : ($now_order_info_c - $pre_order_info_c == 0 ? -1 : 0)
    ...
    public function orderchart()
    {
        ...
        switch ($cycle) {
            case 'thirtyday':
                ...
                $order_list = CrmOrderModel::where('add_time', 'between time', [$datebefor, $dateafter])
                ...
                //统计总数上期
                $pre_total = CrmOrderModel::where('add_time', 'between time', [$pre_datebefor, $pre_dateafter])
                ...
                //统计总数
                $total = CrmOrderModel::where('add_time', 'between time', [$datebefor, $dateafter])
                    ->field("count(*) as count,sum(pay_price) as price")
                    ->find();
                if ($total) {
            ...
            case 'week':
                ...
                $now_order_list = CrmOrderModel::where('add_time', 'between time', [$now_datebefor, $now_dateafter])
                    ->field("FROM_UNIXTIME(add_time,'%w') as day,count(*) as count,sum(pay_price) as price")
                    ->group("FROM_UNIXTIME(add_time, '%Y%m%e')")
                    ->order('add_time asc')
                    ->select()->toArray();
                ...
                //统计总数上期
                $pre_total = CrmOrderModel::where('add_time', 'between time', [$datebefor, $dateafter])
                    ->field("count(*) as count,sum(pay_price) as price")
                    ->find();
                if ($pre_total) {
                    $chartdata['pre_cycle']['count'] = [
                ...
                //统计总数
                $total = CrmOrderModel::where('add_time', 'between time', [$now_datebefor, $now_dateafter])
                    ->field("count(*) as count,sum(pay_price) as price")
                    ->find();
                if ($total) {
                    $cha_count = intval($pre_total['count']) - intval($total['count']);
    ...
    public function Jnotice($newTime = 30)
    {
        header('Content-type:text/json');
        $data = [];
        $data['ordernum'] = CrmOrderModel::statusByWhere(1)->count();//待发货
        ...
        //新订单提醒
        $data['newOrderId'] = CrmOrderModel::statusByWhere(1)->where('is_remind', 0)->column('order_id', 'id');
        if (count($data['newOrderId'])) CrmOrderModel::where('order_id', 'in', $data['newOrderId'])->update(['is_remind' => 1]);
        return app('json')->success('ok', $data);
    }
    function setPlatformConfig($pid){
        if(SystemConfig::where('platform_id',$pid)->count() >0)
            return ;
        $config=new \app\platform\model\system\SystemConfig;
        $list = $config->getTabAll('22,23,24,2,4')->toArray();
        foreach($list as &$li){
            $li['platform_id']=$pid;
            if($li['menu_name'] == 'api'){
                $li['value'] = json_encode(json_decode($li['value'])."?id=".$pid);
            }
        ...
        SystemConfig::beginTrans();
        try {
            $res=SystemConfig::insertAll($list);
        } catch (\Exception $e) {
            SystemConfig::rollbackTrans();
        }
        SystemConfig::checkTrans($res);
