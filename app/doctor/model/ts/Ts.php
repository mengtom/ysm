...
use app\doctor\model\ts\TsMicrochip as MicrochipModel;
/**
 * 配方管理 model
 * Class StoreProduct
 * @package app\doctor\model\ts
 */
class Ts extends BaseModel
{
    ...
    public static function getModelObject($where=[]){
        ...
        if(!empty($where)){
            ...
            $model=$model->group('id')->where(['is_del'=>0]);
            if(isset($where['type'])){
                switch($where['type']){
                    case 1:
                        $model = $model->where('type','In','0,2,3,4');
                    break;
                    case 2:
                    ...
                    default:
                        $model = $model->where('type','In','0,2,3,4');
                        if(isset($where['user_id']) && $where['user_id'] !=''){
                            $model = $model->whereOr('user_id',$where['user_id']);
                        }
            ...
            if(isset($where['keyword']) && $where['keyword']!=''){
                $model = $model->where('code|name_zn|name_en','LIKE',"%{$where['keyword']}%");
            }
            if(isset($where['platform_id']) && $where['platform_id'] !=''){
                $ids=DB::name('crm_platform_ts')->where(['platform_id'=>$where['platform_id'],'status'=>1])->column('ts_id');
                $con=$ids? "id In (".implode(',',$ids).") or platform_id = ".$where['platform_id']:"platform_id = ".$where['platform_id'];
                $model = $model->where($con);
            }
    ...
    public static function TsList($where){
        ...
        if(isset($where['excel']) && (isset($where['limit']) && $where['limit'] > 0 && $where['page'] > 0)) $model=$model->page((int)$where['page'],(int)$where['limit']);
    ...
    public static function TsMicorchipList($where){
        $model=self::getModelObject($where)->field('*,FROM_UNIXTIME(add_time,"%Y-%c-%d %h:%i") as add_time');
        if(isset($where['limit']) && isset($where['page']) && $where['limit'] > 0 && $where['page'] > 0) $model=$model->page((int)$where['page'],(int)$where['limit']);
        $data=($data=$model->select()) && count($data) ? $data->toArray():[];
        foreach ($data as &$item){
