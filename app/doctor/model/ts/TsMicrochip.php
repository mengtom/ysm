...
class TsMicrochip extends BaseModel
{
    ...
    public static function getAll($ts_id){
        $model=new self;
        $model=$model->alias('tm')->join('MicrochipProduct m','m.id = tm.micro_id','right')->where(['tm.ts_id'=>$ts_id])->field(['tm.*,m.zn_name,m.id,m.dose_min,m.dose_max,m.unit,tm.price,tm.usd,m.code,m.is_show'])->select();
