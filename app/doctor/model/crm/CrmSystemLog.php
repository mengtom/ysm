 ...
 * 平台操作记录
 * Class SystemLog
 * @package app\doctor\model\system
 */
class CrmSystemLog extends BaseModel
{
     ...
     */
    public static function doctorVisit($doctorId,$doctorName,$type)
    {
        $request = app('request');
    ...
    /**
     * 手动添加平台当前页面访问记录
     * @param array $doctorInfo
     * @param string $page 页面名称
     * @return object
     */
    public static function setCurrentVisit($doctorInfo, $page)
    {
        $request = app('request');
        $module = app('http')->getName();
        $controller = $request->controller();
    ...
    public static function systemPage($where = array()){
        ...
        if($where['platform_id'] != '')
            $doctorIds = $where['platform_id'];
        else
            $doctorIds = SystemAdmin::where('level','>=',$where['level'])->column('id','id');
        $model = $model->where('l.platform_id','IN',$doctorIds);
        if($where['data'] !== ''){
