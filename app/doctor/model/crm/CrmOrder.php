...
use app\doctor\model\crm\CrmPlatformMicrochip as MicrochipModel;
use app\doctor\model\crm\CrmOrderMicrochip as OrderMicrochip;
use app\doctor\model\wechat\WechatUser;
use app\doctor\model\ump\StorePink;
use app\doctor\model\store\StoreProduct;
...
use app\doctor\model\user\UserBill;

/**
 * 订单管理Model
 * Class StoreOrder
 * @package app\doctor\model\store
 */
class CrmOrder extends BaseModel
{
    ...
    /**
     * 订单状态
     * @param  integer $order_type [订单类型 1 处方 0 采购单]
     * @param  integer $type       [采购类型 1 医生 0 患者]
     * @return [type]              [description]
     */
    public static function orderCount($order_type = 0,$type=0)
    {
        $data['wz'] = self::statusByWhere(0, new self())->where(['is_system_del' => 0,'order_type'=>$order_type,'order_type_cate'=>$type])->count();
        $data['wf'] = self::statusByWhere(1, new self())->where(['is_system_del' => 0, 'shipping_type' => 1,'order_type'=>$order_type,'order_type_cate'=>$type])->count();
        $data['ds'] = self::statusByWhere(2, new self())->where(['is_system_del' => 0, 'shipping_type' => 1,'order_type'=>$order_type,'order_type_cate'=>$type])->count();
        $data['dp'] = self::statusByWhere(3, new self())->where(['is_system_del' => 0,'order_type'=>$order_type,'order_type_cate'=>$type])->count();
        $data['jy'] = self::statusByWhere(4, new self())->where(['is_system_del' => 0,'order_type'=>$order_type,'order_type_cate'=>$type])->count();
        $data['tk'] = self::statusByWhere(-1, new self())->where(['is_system_del' => 0,'order_type'=>$order_type,'order_type_cate'=>$type])->count();
        $data['yt'] = self::statusByWhere(-2, new self())->where(['is_system_del' => 0,'order_type'=>$order_type,'order_type_cate'=>$type])->count();
        $data['del'] = self::statusByWhere(-4, new self())->where(['is_system_del' => 0,'order_type'=>$order_type,'order_type_cate'=>$type])->count();
        $data['write_off'] = self::statusByWhere(5, new self())->where(['is_system_del' => 0,'order_type'=>$order_type,'order_type_cate'=>$type])->count();
        $data['general'] = self::where(['pink_id' => 0, 'combination_id' => 0, 'seckill_id' => 0, 'bargain_id' => 0, 'is_system_del' => 0,'order_type'=>$order_type,'order_type_cate'=>$type])->count();
        $data['pink'] = self::where('pink_id|combination_id', '>', 0)->where(['is_system_del'=> 0,'order_type'=>$order_type,'order_type_cate'=>$type])->count();
        $data['seckill'] = self::where('seckill_id', '>', 0)->where(['is_system_del'=> 0,'order_type'=>$order_type,'order_type_cate'=>$type])->count();
        $data['bargain'] = self::where('bargain_id', '>', 0)->where(['is_system_del'=> 0,'order_type'=>$order_type,'order_type_cate'=>$type])->count();
        return $data;
    }
    ...
    public static function OrderList($where)
    {
        ...
        foreach ($data as &$item) {
            ...
            $item['count_total_price']=$item['total_price'] * ($item['taking_quency'] >0? $item['taking_quency']:1) * ($item['taking_cycle'] >0 ?$item['taking_cycle']:1);
...
HTML;
    ...
    public static function getOrderWhere($where, $model, $aler = '', $join = '')
    {
        ...
        if(isset($where['doctor']) && $where['doctor'] !=''){
            $model=$model->where($aler.'doctor_leader',$where['doctor']);
        }
        ...
        if(isset($where['type']) && $where['type'] >=0){
            $model=$model->where($aler.'order_type_cate',$where['type']);
        }
        if(isset($where['order_type']) && ($where['order_type'] !='' || $where['order_type'] === 0)){
            $model=$model->where($aler.'order_type',$where['order_type']);
        }
...
//        if($where['is_del'] != '' && $where['is_del'] != -1) $model = $model->where('is_del',$where['is_del']);
     ...
     * @throws \think\exception\DbException
     */
    public static function cacheKeyCreatePrescripOrder($uid,$key, $micro,$platform_id,$real_name,$user_phone,$order_name,$symptom,$taking_quency,$taking_time,$taking_cycle=0,$taking_suggest='',$taking_remark = '', $institu_id = 0,$doctor_id=0,$test=false)
    {
        self::beginTrans();
        try {
            ...
            $price=0;
            foreach($microchipInfo['data'] as $v){
                $price +=sprintf("%.2f",$v['sell_price'] * $micro[$v['id']]);
            }
            ...
            $taking_cycle=$taking_cycle > 0 ? $taking_cycle:28;   //周期
            $taking_quency=$taking_quency > 0? $taking_quency:1;  //次数
            $total_price=round($price*$taking_cycle*$taking_quency,2);     //总价
            if ($test) {
                self::rollbackTrans();
                return [
            ...
            if (!$order) return self::setErrorInfo('处方生成失败!', true);
            //保存商品信息
            $res4 = false !== OrderMicrochip::setOrderMicrochipInfo($order['id'], $microchipInfo['data'],$micro);

            if (!$res4 ) return self::setErrorInfo('处方生成失败!', true); //|| !$res5 || !$res6
            //自动设置默认地址
            // UserRepository::storeProductOrderCreateEbApi($order, compact('cartInfo', 'addressId'));
            // self::clearCacheOrderInfo($uid, $key);
            self::commitTrans();
            // StoreOrderStatus::status($order['id'], 'cache_key_create_order', '处方生成');
            return $order;
        } catch (\PDOException $e) {
            self::rollbackTrans();
            return self::setErrorInfo('生成处方时SQL执行错误错误原因：' . $e->getMessage());
        } catch (\Exception $e) {
            self::rollbackTrans();
            return self::setErrorInfo('生成处方时系统错误错误原因：' . $e->getMessage());
        }
    }
    /**
     * 生成采购单
    ...
    public static function cacheKeyCreatePurcharseOrder($uid,$key, $micro,$platform_id,$real_name,$user_phone,$order_name,$idcard,$taking_quency,$taking_cycle,$taking_time=0,$taking_suggest='',$taking_remark = '', $user_phone2,$institu_id = 0,$doctor_id=0,$test=false)
    {
        ...
        try {
            ...
            $total_num=array_sum($micro);
            // $price=sprintf("%.2f",$micro_price['rmb_price']*$total_num);
            $real_name=filterEmoji($real_name);
            $order_name=htmlspecialchars($order_name);
            // $symptom=htmlspecialchars($symptom);
            $taking_remark=htmlspecialchars($taking_remark);
            ...
            $orderInfo = [
                ...
                'unique' => $key,
                'combination_id'=>'采购单',
                'order_type_cate'=>$userInfo['type']? 1: 0,
                'idcard'=>$idcard,
            ];
            $order = self::create($orderInfo);
...
HTML;
        ...
        $order['count_total_price']=$order['total_price'] * ($order['taking_quency'] >0? $order['taking_quency']:1) * ($order['taking_cycle'] >0 ?$order['taking_cycle']:1);
        return $order;
    }
}