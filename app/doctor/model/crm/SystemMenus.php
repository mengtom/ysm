...
class SystemMenus extends BaseModel
{
    ...
    {
        $menusList = self::where('is_show','1')->where('module','doctor')->where('access','1')->order('sort DESC')->select();
    ...
    public static function tidyMenuTier($platformFilter = false,$menusList,$pid = 0,$navList = [])
    {
        ...
        if($platformAuth === null) $platformAuth = $platformFilter == true ? CrmUsers::activeDoctorAuthOrFail() : [];//当前登录用户的菜单
