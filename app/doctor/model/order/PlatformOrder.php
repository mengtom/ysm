...
class PlatformOrder extends BaseModel
{
    ...
    public static function OrderList($where)
    {
        ...
        foreach ($data as &$item) {
            ...
            // }
            // $item['_info'] = $_info;
            $micro=Db::name('crm_order_microchip')->where('oid', $item['id'])->field('micro_code,micro_name,micro_price,single_price,num,total_dose')->select();
            $item['_micro']=$micro;
            $item['micro_num']=count($micro);
            $item['micro_total']=Db::name('crm_order_microchip')->where('oid', $item['id'])->field('num')->count();
