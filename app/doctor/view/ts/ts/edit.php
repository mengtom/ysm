{include file="public/header"}
	...
	<div id="app" class="ts-edit pr-20 pl-20 mw-1160">
		<div class="w-100 tip mt-20  text-666 size-14">
			<span>首页 /</span> <a class="text-57" href="index.html"> 配方管理 / </a> <span class="text-0081a7"> 添加自有配方 </span>
		</div>
		<div class="w-100 fw size-20 mt-10 text-27">添加配方</div>
		<!--列表-->
		<div class="w-100  mt-10 pb-10 mb-30" style="border-radius: 1px;">
			<form class="d-flex w-100 layui-form  align-items-start justify-content-between" id="formput" action="{:Url('save',array('id'=>$info['id']))}" method="post">
				<div class="wp-edit-body h-100 pt-20 bg-fff">
					...
					<div class="items pl-30 w-100">
						<div class="item w-100 mt-20">
							...
							<div class="w-100 mt-8">
								<input type="text" placeholder="由英文/数字/符号组成" class="ivu-input ml0 w-400px" name="code" value="{$info.code}">
							</div>
						</div>
						...
						<div class="item w-100 d-flex align-items-center justify-content-start mt-20">
							<div>
								...
								<div class="w-100 mt-8">
									<input type="text" placeholder="由中文/英文/数字/符号组成" class="ivu-input w-400px" name="name_zn" value="{$info.name_zn}">
								</div>
							</div>
							<div style="margin-left: 40px;">
								<div class="size-14 item-text">Name</div>
								<div class="w-100 mt-8">
									<input type="text" placeholder="由英文/数字/符号组成" class="ivu-input w-400px" name="name_en" value="{$info.name_en}">
								</div>
							</div>
						</div>

						<div class="item w-100 d-flex align-items-start justify-content-start mt-20">
							<div>
								<div class="size-14 item-text">配方状态</div>
								<div class="w-100 mt-8">
									<i-switch {if condition="$info.is_status"} :value="true"{else/} :value="false"{/if} size="large" name="is_status">
					...
					<div class="size-16 fw pl-30 text-0081a7">微片选择
					...
					<div class="items w-100 pr-30 pl-30">
						...
						<div class="w-100 d-flex align-items-center justify-content-start flex-column  hxcflist">
							...
							<div v-for="(i,k) in hxcflistdata" class="w-100 d-flex align-items-center justify-content-around hxcfitem" style="height: 40px;">
								...
								<div class="text-left ellipsis-1 w-200px">{{i.name}}</div>
                                ...
                                <div class="d-flex align-items-center justify-content-center w-50px">
									<i class="icon iconfont iconcha text-0081a7 cursor rmhxcfitem" :data-key="k" :data-id="i.id" @click="itemsdel(i,k)"></i>
								</div>
							</div>
						</div>
					...
					<div class="size-16 fw text-0081a7 pl-30" style="margin-top: 18px;">医嘱</div>
						...
						<div>
							...
							<div class="w-100 mt-8">
								<div class="relative w-400px">
									<i-select v-model="model18" filterable multiple allow-create @on-create="handleCreate2" name="cate_id">
					...
					<div class="item w-100 mt-20 pl-30 d-flex align-items-center justify-content-start">
						<div class="w-400px d-flex align-items-start justify-content-center">
								...
								<div class="w-100 mt-8">
									<div class="ivu-select  ivu-select-single ivu-select-default relative w-190px">
										<select id="frequency" lay-filter="frequency" class="frequency" lay-verify="required" name="taking_quency">
											...
											<option value="1" {eq name="1" value="$info.taking_quency"}selected="selected"{/eq}>一日一次</option>
											<option value="2" {eq name="2" value="$info.taking_quency"}selected="selected"{/eq}>早晚各一次</option>
											<!--<option value="2" {eq name="2" value="$info.taking_quency"}selected="selected"{/eq}>一日二次</option>
											<option value="3" {eq name="3" value="$info.taking_quency"}selected="selected"{/eq}>一日三次</option>-->
										</select>
									</div>
								</div>
							</div>
							<div class="w-190px ml-20">
								...
								<div class="w-100 mt-8">
									<div class="ivu-select  ivu-select-single ivu-select-default relative w-190px">
										<select name="taking_time" id="takedate" class="takedate" lay-filter="takedate">
											<option value="">请选择</option>
											<option value="无建议"  {eq name="'无建议'" value="$info.taking_time"}selected="selected"{/eq}>无建议</option>
											<option value="早上" {eq name="'早上'" value="$info.taking_time"}selected="selected"{/eq}>早上</option>
											<option value="下午" {eq name="'下午'" value="$info.taking_time"}selected="selected"{/eq}>下午</option>
										</select>
									</div>
								</div>
							</div>
						...
						<div style="margin-left: 40px;" class="w-400px ">
							...
							<div class="w-100 mt-8">
								<div class="ivu-select  ivu-select-single ivu-select-default relative w-400px">
									<select name="taking_cycle" id="period" class="period" lay-filter="period" lay-verify="required">
										<option value="">请选择</option>
										<option value="28" {eq name="'28'" value="$info.taking_cycle"}selected="selected"{/eq}>28天</option>
										<option value="56" {eq name="'56'" value="$info.taking_cycle"}selected="selected"{/eq}>56天</option>
										<option value="84" {eq name="'84'" value="$info.taking_cycle"}selected="selected"{/eq}>84天</option>
										<!--<option value="10" {eq name="'10'" value="$info.taking_cycle"}selected="selected"{/eq}>10天</option>
										<option value="20" {eq name="'20'" value="$info.taking_cycle"}selected="selected"{/eq}>20天</option>
										<option value="30" {eq name="'30'" value="$info.taking_cycle"}selected="selected"{/eq}>30天</option>
										<option value="60" {eq name="'60'" value="$info.taking_cycle"}selected="selected"{/eq}>60天</option>
										<option value="90" {eq name="'90'" value="$info.taking_cycle"}selected="selected"{/eq}>90天</option>-->
									</select>
								</div>
							</div>
						</div>
					...
					<div class="size-16 pl-30 fw text-0081a7" style="margin-top: 18px;">描述和参考</div>

					<div class="item w-100 pl-30 mt-20 d-flex align-items-start justify-content-start">
						<div>
							<div class="size-14 item-text">描述</div>
							<div class="w-100 mt-8">
								<textarea rows="3" placeholder="请输入" class="ivu-input w-400px" name="description"> {$info.description}</textarea>
							</div>
						</div>
						<div style="margin-left: 50px;">
							<div class=" size-14 item-text">参考</div>
							<div class="w-100 mt-8">
								<textarea rows="3" placeholder="请输入" class="ivu-input w-400px" name="reference"> {$info.reference}</textarea>
							</div>
						</div>
					</div>
				...
				<div class="ts_right d-flex ml-20 h-auto d-flex align-items-start justify-content-start flex-column">
					<div class="top w-100 bg-fff h-auto pb-10">
					...
					<div class="mt-20 cf-list w-100 bg-fff">
						...
						<div class="items w-100 d-flex align-items-center justify-content-center flex-column mb-30">
							<div v-for="i in cflist" class="item  d-flex align-items-center justify-content-between">
								<div class="ellipsis-1" :title="i.name">{{i.name}}</div>
								<div>{{i.dose}}{{i.unit}}</div>
							</div>
						</div>
					</div>
					<div class="mt-10 w-100 d-flex align-items-center justify-content-between">
						<button class="ivu-btn ivu-btn-primary" name="style" value="1" style="width: 130px;" >
							<i class="icon iconfont iconyishenghoutai-chufang1"></i>
							直接生成处方
						</button>
						<button style="width: 130px;" class="ivu-btn ivu-btn-primary" type="submit" name="style" value="2">
							<i class="icon size-10 iconfont iconyishenghoutai-gouwuche1"></i>
							采购本配方
						</button>
					</div>
		...
		<div style="display: none;" class="modal-root ts-edit-modal">
			...
			<div class="ant-modal fixed" style="width : 852px ;height: 520px ;">
				...
				<div class="items w-100" style="height: 520px;margin-top: 0;">
					<iframe id="wpitems" src="{:Url('microchip_selected',array('type'=>$type,'time'=>time()))}" class="w-100 h-100" style="border: none"></iframe>
				</div>
...
<script type="text/javascript">
	var app = new Vue({
		...
		data   : {
			...
			suggest     : [
				{volist name="$info.taking_suggest" id="s"}
				{
					value: '{$s}',
					label: '{$s}',
				},
	...
	* frequency       服用频次
	* */
	var takedate = 0, period = {$info.taking_cycle}, frequency = {$info.taking_quency};
	layui.use(['form'], function () {
		...
		form.on('select(period)', function (data) {console.log(data)
			setTimeout(function () {
				total();
			}, 1000);
	...
	* */
	function total() {
		...
		var map = {}, dest = [], singlepricermb = 0,  periodpricermb = 0,  wpkind = 0, hxkind = 0, flkind = 0, ids = [];
	...
	* */
	...
	// 	var key          = $(this).data(key);
	// 	var hxcflistdata = app.$data.hxcflistdata;
	// 	hxcflistdata.splice(key, 1);
	// 	app.$data.hxcflistdata = hxcflistdata;
	// 	total();
	// });
