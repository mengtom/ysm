 ...
 * @day: 2017/11/11
 */
namespace app\doctor\controller\crm;

use app\doctor\controller\AuthController;
// use app\doctor\model\system\Express;
...
use app\doctor\model\order\StoreOrderStatus;
...
use app\doctor\model\crm\CrmOrder as CrmOrderModel;
/**
...
class CrmOrder extends AuthController
{
    ...
    public function index()
    {
        ...
        $this->assign([
            ...
            'orderCount'=>CrmOrderModel::orderCount(1),
    ...
    public function order()
    {
        ...
        $this->assign([
            ...
            'status'=>$this->request->param('status',''),
            'orderCount'=>CrmOrderModel::orderCount(0,$type),
            'payTypeCount'=>CrmOrderModel::payTypeCount(),
        ]);
        return $this->fetch();
    }
...
//        $f[] = Form::radio('status','状态',$product->getData('status'))->options([['label'=>'开启','value'=>1],['label'=>'关闭','value'=>0]]);
    ...
    public function savePurchase(){
        ...
        $doVa=$OrderValidate->scene('purchase')->check($data);
        if(!$doVa){
            return $this->failed($OrderValidate->getError(),$_SERVER['HTTP_REFERER']);
        }
        $user_id=$this->doctorId;
        $key=CrmOrderModel::createOrderKey('cf',$user_id,time());
        // $order_key=CrmOrderModel::createOrderKey('cf',$user_id,$order_sn);
        $order=CrmOrderModel::cacheKeyCreatePurcharseOrder($user_id,$key,$micro,$this->doctorInfo['platform_leader'],$data['real_name'],$data['user_phone'],$data['order_name'],$data['idcard'],$data['taking_quency'],$data['taking_time'],$data['taking_cycle'],$data['taking_suggest'],$data['taking_remark'],$data['user_phone2'],$this->doctorInfo['institu_leader'],$this->doctorDId);
        if(!$order)
            return $this->failed(CrmOrderModel::getErrorInfo());
        $paytype=$this->request->param('paytype','');
        if($paytype){

        }
        return $this->successful('采购单生成成功!');
    }
