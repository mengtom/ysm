...
class CrmDoctor extends AuthController
{
    ...
    public function save()
    {
        ...
        try{
            validate(DoctorValidate::class)->check($data);
        ...
        try {
            $default=CrmUsers::addUsers(['account'=>$data['account'],'password'=>$data['password'],'type'=>1,'platform_leader'=>$data['platform_leader'],'platform_name'=>$this->platformInfo['p_name'],'last_ip'=>app('request')->ip(),'last_login'=>time(),'group'=>$data['group'],'group_discount'=>$group['discount'],'group_commission'=>$group['commission'],'group_name'=>$group['name']]);
        } catch (\Exception $e) {
        ...
        BaseModel::rollbackTrans();
        return $this->failed(DoctorModel::getErrorInfo('添加机构失败'));
    }
    ...
    public function update($id)
    {
        ...
        try{
            validate(DoctorValidate::class)->scene('basic')->check($data);
        }catch(ValidateException $e){
