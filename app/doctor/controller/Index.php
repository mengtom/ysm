...
use app\doctor\model\crm\CrmOrder as CrmOrderModel;//订单
use app\doctor\model\user\User;
use app\doctor\model\user\UserExtract as UserExtractModel;//分销
use app\doctor\model\crm\CrmUsers as CrmUserModel;//用户
use app\doctor\model\store\StoreProductReply as StoreProductReplyModel;//评论
use app\doctor\model\crm\CrmPlatformMicrochip as MicrochipProductModel;//产品
use app\doctor\model\ts\Ts as TsModel;//配方
...
class Index extends AuthController
{
    ...
    public function main()
    {
        ...
        $topData['name']=$this->doctorInfo['name'];
        $topData['now_day']=date('Y年m月d日');
        // $pre_day = strtotime(date('Y-m-d', strtotime('-1 day')));//昨天时间戳
        // $beforyester_day = strtotime(date('Y-m-d', strtotime('-2 day')));//前天时间戳
        //患者数量
        $topData['patientCount']=CrmUserModel::where(['is_del'=>0,'doctor_leader'=>$this->doctorDId,'type'=>0])->count();
        //处方
        $topData['prescriptCount']=CrmOrderModel::getOrderWhere(['doctor'=>$this->doctorDId,'order_type'=>1],new CrmOrderModel)->count();
        //采购单
        $topData['orderCount']=CrmOrderModel::getOrderWhere(['doctor'=>$this->doctorDId,'type'=>0],new CrmOrderModel)->count();
        //微片
        $list['microchip']=MicrochipProductModel::getSelectMicrochip(['page'=>1,'limit'=>4,'platform'=>$this->doctorInfo['platform_leader']]);
        //参考配方
        $list['ts_ts']=TsModel::TsList(['page'=>1,'limit'=>4,'excel'=>0,'type'=>'1','platform_id'=>$this->doctorInfo['platform_leader']]);
        //我的配方
        $list['ts']=TsModel::TsList(['page'=>1,'limit'=>4,'excel'=>0,'type'=>'2','platform_id'=>$this->doctorInfo['platform_leader'],'user_id'=>$this->doctorId]);
        //处方
        $list['order'] =CrmOrderModel::OrderList(['page'=>1,'limit'=>4,'order_type'=>1,'doctor'=>$this->doctorDId]);
