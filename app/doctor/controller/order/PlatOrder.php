 ...
 * @day: 2017/11/11
 */
namespace app\platform\controller\order;
...
use app\platform\model\order\StoreOrderStatus;
...
use app\platform\model\order\PlatformOrder as PlatformOrderModel;
/**
 * 订单管理控制器 同一个订单表放在一个控制器
 * Class StoreOrder
 * @package app\platform\controller\store
 */
class PlatOrder extends AuthController
{
    /**
     * @return mixed
     */
    public function index()
    {
        $this->assign([
            ...
            'orderCount'=>PlatformOrderModel::orderCount(),
            'payTypeCount'=>PlatformOrderModel::payTypeCount(),
        ]);
        return $this->fetch();
    }
    /**
    ...
    public function getBadge(){
        ...
        return JsonService::successful(PlatformOrderModel::getBadge($where));
    }
    /**
     * 获取订单列表
     * return json
    ...
    public function order_list(){
        ...
        return JsonService::successlayui(PlatformOrderModel::OrderList($where));
    }
    ...
    public function write_order($verify_code = '',$is_confirm = 0)
    {
        if($this->request->isAjax()){
            ...
            PlatformOrderModel::beginTrans();
            $orderInfo = PlatformOrderModel::where('verify_code',$verify_code)->where('paid',1)->where('refund_status',0)->find();
            ...
            if($orderInfo->save()) {
                ...
                PlatformOrderModel::commitTrans();
                return JsonService::successful('核销成功！');
            }else {
                PlatformOrderModel::rollbackTrans();
    ...
    public function orderchart(){
        ...
        $orderCount = [
            urlencode('未支付')=>PlatformOrderModel::getOrderWhere($where,PlatformOrderModel::statusByWhere(0))->count(),
            urlencode('未发货')=>PlatformOrderModel::getOrderWhere($where,PlatformOrderModel::statusByWhere(1))->count(),
            urlencode('待收货')=>PlatformOrderModel::getOrderWhere($where,PlatformOrderModel::statusByWhere(2))->count(),
            urlencode('待评价')=>PlatformOrderModel::getOrderWhere($where,PlatformOrderModel::statusByWhere(3))->count(),
            urlencode('交易完成')=>PlatformOrderModel::getOrderWhere($where,PlatformOrderModel::statusByWhere(4))->count(),
            urlencode('退款中')=>PlatformOrderModel::getOrderWhere($where,PlatformOrderModel::statusByWhere(-1))->count(),
            urlencode('已退款')=>PlatformOrderModel::getOrderWhere($where,PlatformOrderModel::statusByWhere(-2))->count()
        ];
        $model = PlatformOrderModel::getOrderWhere($where,new PlatformOrderModel())->field('sum(total_num) total_num,count(*) count,sum(total_price) total_price,sum(refund_price) refund_price,from_unixtime(add_time,\'%Y-%m-%d\') add_time')
        ...
        $this->assign(PlatformOrderModel::systemPage($where,$this->platformPId));
        $this->assign('price',PlatformOrderModel::getOrderPrice($where));
    ...
    public function edit($id)
    {
        ...
        $product = PlatformOrderModel::get($id);
...
//        $f[] = Form::radio('status','状态',$product->getData('status'))->options([['label'=>'开启','value'=>1],['label'=>'关闭','value'=>0]]);
    ...
    public function update($id)
    {
        ...
        $data['order_id'] = PlatformOrderModel::changeOrderId($data['order_id']);
        PlatformOrderModel::edit($data,$id);
    ...
    public function del_order()
    {
        ...
        if(PlatformOrderModel::where('is_del',0)->where('id','in',$ids)->count()) return JsonService::fail('您选择的的订单存在用户未删除的订单，无法删除用户未删除的订单');
        $res=PlatformOrderModel::where('id','in',$ids)->update(['is_system_del'=>1]);
        if($res)
    ...
    public function update_delivery($id = 0){
        ...
        //短信发送
        event('ShortMssageSend',[PlatformOrderModel::where('id',$id)->value('order_id'),'Deliver']);
        return Json::successful('修改成功!');
    }
    ...
    public function take_delivery($id){
        if(!$id) return $this->failed('数据不存在');
        $order = PlatformOrderModel::get($id);
        ...
        PlatformOrderModel::beginTrans();
        try{
            if(!PlatformOrderModel::edit($data,$id)) {
                PlatformOrderModel::rollbackTrans();
                return Json::fail(PlatformOrderModel::getErrorInfo('收货失败,请稍候再试!'));
            }else{
    ...
    public function updateRefundY($id){
        ...
        $resEdit = PlatformOrderModel::edit($data,$id);
        $res = true;
        if($resEdit){
            $data['type'] = $type;
    ...
    {
        if(!$oid || !($orderInfo = PlatformOrderModel::get($oid)))
    ...
    {
        if(!$oid || !($order = PlatformOrderModel::get($oid)))
            return $this->failed('订单不存在!');
    ...
    public function offline($id){
        $res = PlatformOrderModel::updateOffline($id);
        if($res){
            event('StoreProductOrderOffline',[$id]);
            StoreOrderStatus::setStatus($id,'offline','线下付款');
            return Json::successful('修改成功!');
        }else{
            return Json::fail(PlatformOrderModel::getErrorInfo('修改失败!'));
        }
    }
    /**
     * 修改积分和金额
